...
class TreeInterpreter
{
    ...
    {
        $this->fnDispatcher = $fnDispatcher ?: \WPMailSMTP\Vendor\JmesPath\FnDispatcher::getInstance();
    }
    /**
    ...
    private function dispatch(array $node, $value)
    {
        ...
        switch ($node['type']) {
            ...
            case 'index':
                if (!\WPMailSMTP\Vendor\JmesPath\Utils::isArray($value)) {
                    return null;
                }
            ...
            case 'projection':
                ...
                switch ($node['from']) {
                    case 'object':
                        if (!\WPMailSMTP\Vendor\JmesPath\Utils::isObject($left)) {
                            return null;
                        }
                        break;
                    case 'array':
                        if (!\WPMailSMTP\Vendor\JmesPath\Utils::isArray($left)) {
                            return null;
                        }
                        break;
                    default:
            ...
            case 'or':
                ...
                return \WPMailSMTP\Vendor\JmesPath\Utils::isTruthy($result) ? $result : $this->dispatch($node['children'][1], $value);
            case 'and':
                $result = $this->dispatch($node['children'][0], $value);
                return \WPMailSMTP\Vendor\JmesPath\Utils::isTruthy($result) ? $this->dispatch($node['children'][1], $value) : $result;
            case 'not':
                return !\WPMailSMTP\Vendor\JmesPath\Utils::isTruthy($this->dispatch($node['children'][0], $value));
            case 'pipe':
                return $this->dispatch($node['children'][1], $this->dispatch($node['children'][0], $value));
            ...
            case 'comparator':
                ...
                if ($node['value'] == '==') {
                    return \WPMailSMTP\Vendor\JmesPath\Utils::isEqual($left, $right);
                } elseif ($node['value'] == '!=') {
                    return !\WPMailSMTP\Vendor\JmesPath\Utils::isEqual($left, $right);
                } else {
            ...
            case 'condition':
                return \WPMailSMTP\Vendor\JmesPath\Utils::isTruthy($this->dispatch($node['children'][0], $value)) ? $this->dispatch($node['children'][1], $value) : null;
            case 'function':
                $args = [];
            ...
            case 'slice':
                return \is_string($value) || \WPMailSMTP\Vendor\JmesPath\Utils::isArray($value) ? \WPMailSMTP\Vendor\JmesPath\Utils::slice($value, $node['value'][0], $node['value'][1], $node['value'][2]) : null;
            case 'expref':
