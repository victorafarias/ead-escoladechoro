...
class DecodingEventStreamIterator implements \Iterator
{
    ...
    protected function parseHeaders($headerBytes)
    {
        ...
        while ($bytesRead < $headerBytes) {
            ...
            if (isset($headers[$key])) {
                throw new \WPMailSMTP\Vendor\Aws\Api\Parser\Exception\ParserException('Duplicate key in event headers.');
            }
            $headers[$key] = $value;
        }
    ...
    protected function parsePrelude()
    {
        ...
        if (\unpack('N', $calculatedCrc)[1] !== $prelude[self::CRC_PRELUDE]) {
            throw new \WPMailSMTP\Vendor\Aws\Api\Parser\Exception\ParserException('Prelude checksum mismatch.');
        }
    ...
    protected function parseEvent()
    {
        ...
        if ($this->stream->tell() < $this->stream->getSize()) {
            ...
            if ($prelude[self::LENGTH_TOTAL] > $bytesLeft) {
                throw new \WPMailSMTP\Vendor\Aws\Api\Parser\Exception\ParserException('Message length too long.');
            }
            $bytesLeft -= $numBytes;
    ...
    private function decodeBytes($lengthBytes = 2)
    {
        if (!isset(self::$lengthFormatMap[$lengthBytes])) {
            throw new \WPMailSMTP\Vendor\Aws\Api\Parser\Exception\ParserException('Undefined variable length format.');
        }
    ...
    private function decodeTimestamp()
    {
        ...
        return [\WPMailSMTP\Vendor\Aws\Api\DateTimeResult::createFromFormat('U.u', $val / 1000), $bytes];
    }
