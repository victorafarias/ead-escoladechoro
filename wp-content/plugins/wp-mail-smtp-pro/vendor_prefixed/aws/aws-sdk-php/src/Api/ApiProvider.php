...
use WPMailSMTP\Vendor\Aws\Exception\UnresolvedApiException;
/**
 * API providers.
 *
...
class ApiProvider
{
    ...
    public static function resolve(callable $provider, $type, $service, $version)
    {
        ...
        throw new \WPMailSMTP\Vendor\Aws\Exception\UnresolvedApiException($msg);
    }
    /**
    ...
    {
        return new self(__DIR__ . '/../data', \WPMailSMTP\Vendor\Aws\manifest());
    }
    /**
    ...
    public function __invoke($type, $service, $version)
    {
        ...
        try {
            return \WPMailSMTP\Vendor\Aws\load_compiled_json($path);
