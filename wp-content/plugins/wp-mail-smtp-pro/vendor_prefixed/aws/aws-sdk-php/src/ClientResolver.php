...
use WPMailSMTP\Vendor\Aws\Auth\AuthResolver;
use WPMailSMTP\Vendor\Aws\Auth\AuthSchemeResolver;
...
use WPMailSMTP\Vendor\Aws\ClientSideMonitoring\Configuration;
use WPMailSMTP\Vendor\Aws\Configuration\ConfigurationResolver;
use WPMailSMTP\Vendor\Aws\Credentials\CredentialProvider;
use WPMailSMTP\Vendor\Aws\Credentials\Credentials;
use WPMailSMTP\Vendor\Aws\Credentials\CredentialsInterface;
use WPMailSMTP\Vendor\Aws\DefaultsMode\ConfigurationInterface as ConfigModeInterface;
use WPMailSMTP\Vendor\Aws\DefaultsMode\ConfigurationProvider as ConfigModeProvider;
use WPMailSMTP\Vendor\Aws\Endpoint\EndpointProvider;
use WPMailSMTP\Vendor\Aws\Endpoint\PartitionEndpointProvider;
use WPMailSMTP\Vendor\Aws\Endpoint\UseDualstackEndpoint\Configuration as UseDualStackEndpointConfiguration;
use WPMailSMTP\Vendor\Aws\Endpoint\UseDualstackEndpoint\ConfigurationInterface as UseDualStackEndpointConfigurationInterface;
use WPMailSMTP\Vendor\Aws\Endpoint\UseDualstackEndpoint\ConfigurationProvider as UseDualStackConfigProvider;
use WPMailSMTP\Vendor\Aws\Endpoint\UseFipsEndpoint\Configuration as UseFipsEndpointConfiguration;
use WPMailSMTP\Vendor\Aws\Endpoint\UseFipsEndpoint\ConfigurationInterface as UseFipsEndpointConfigurationInterface;
use WPMailSMTP\Vendor\Aws\Endpoint\UseFipsEndpoint\ConfigurationProvider as UseFipsConfigProvider;
use WPMailSMTP\Vendor\Aws\EndpointDiscovery\ConfigurationInterface;
use WPMailSMTP\Vendor\Aws\EndpointDiscovery\ConfigurationProvider;
use WPMailSMTP\Vendor\Aws\EndpointV2\EndpointDefinitionProvider;
use WPMailSMTP\Vendor\Aws\Exception\AwsException;
use WPMailSMTP\Vendor\Aws\Exception\InvalidRegionException;
use WPMailSMTP\Vendor\Aws\Retry\ConfigurationInterface as RetryConfigInterface;
use WPMailSMTP\Vendor\Aws\Retry\ConfigurationProvider as RetryConfigProvider;
use WPMailSMTP\Vendor\Aws\Signature\SignatureProvider;
use WPMailSMTP\Vendor\Aws\Token\Token;
use WPMailSMTP\Vendor\Aws\Token\TokenInterface;
use WPMailSMTP\Vendor\Aws\Token\TokenProvider;
...
class ClientResolver
{
     ...
     */
    public function resolve(array $args, \WPMailSMTP\Vendor\Aws\HandlerList $list)
    {
        $args['config'] = [];
    ...
    public static function _apply_retries($value, array &$args, \WPMailSMTP\Vendor\Aws\HandlerList $list)
    {
        // A value of 0 for the config option disables retries
        if ($value) {
            $config = \WPMailSMTP\Vendor\Aws\Retry\ConfigurationProvider::unwrap($value);
            if ($config->getMode() === 'legacy') {
                // # of retries is 1 less than # of attempts
                $decider = \WPMailSMTP\Vendor\Aws\RetryMiddleware::createDefaultDecider($config->getMaxAttempts() - 1);
                $list->appendSign(\WPMailSMTP\Vendor\Aws\Middleware::retry($decider, null, $args['stats']['retries']), 'retry');
            } else {
                $list->appendSign(\WPMailSMTP\Vendor\Aws\RetryMiddlewareV2::wrap($config, ['collect_stats' => $args['stats']['retries']]), 'retry');
            }
        }
    }
    public static function _apply_defaults($value, array &$args, \WPMailSMTP\Vendor\Aws\HandlerList $list)
    {
        $config = \WPMailSMTP\Vendor\Aws\DefaultsMode\ConfigurationProvider::unwrap($value);
        if ($config->getMode() !== 'legacy') {
    ...
    public static function _apply_disable_request_compression($value, array &$args)
    {
        ...
        if (!\is_bool($value)) {
            throw new \InvalidArgumentException("Invalid configuration value provided for 'disable_request_compression'." . " value must be a bool.");
        }
    ...
    public static function _apply_min_compression_size($value, array &$args)
    {
        ...
        if (!\is_int($value) || \is_int($value) && ($value < 0 || $value > 10485760)) {
            throw new \InvalidArgumentException(" Invalid configuration value provided for 'min_compression_size_bytes'." . " value must be an integer between 0 and 10485760, inclusive.");
        }
        $args['config']['request_min_compression_size_bytes'] = $value;
    }
    public static function _default_min_compression_size(array &$args)
    {
        return \WPMailSMTP\Vendor\Aws\Configuration\ConfigurationResolver::resolve('request_min_compression_size_bytes', 10240, 'int', $args);
    }
    public static function _apply_credentials($value, array &$args)
    {
        ...
        if ($value instanceof \WPMailSMTP\Vendor\Aws\Credentials\CredentialsInterface) {
            $args['credentials'] = \WPMailSMTP\Vendor\Aws\Credentials\CredentialProvider::fromCredentials($value);
        } elseif (\is_array($value) && isset($value['key']) && isset($value['secret'])) {
            $args['credentials'] = \WPMailSMTP\Vendor\Aws\Credentials\CredentialProvider::fromCredentials(new \WPMailSMTP\Vendor\Aws\Credentials\Credentials($value['key'], $value['secret'], $value['token'] ?? null, $value['expires'] ?? null, $value['accountId'] ?? null));
        } elseif ($value === \false) {
            $args['credentials'] = \WPMailSMTP\Vendor\Aws\Credentials\CredentialProvider::fromCredentials(new \WPMailSMTP\Vendor\Aws\Credentials\Credentials('', ''));
            $args['config']['signature_version'] = 'anonymous';
            $args['config']['configured_signature_version'] = \true;
        } elseif ($value instanceof \WPMailSMTP\Vendor\Aws\CacheInterface) {
            $args['credentials'] = \WPMailSMTP\Vendor\Aws\Credentials\CredentialProvider::defaultProvider($args);
        } else {
            throw new \InvalidArgumentException('Credentials must be an instance of ' . "'" . \WPMailSMTP\Vendor\Aws\Credentials\CredentialsInterface::class . ', an associative ' . 'array that contains "key", "secret", and an optional "token" ' . 'key-value pairs, a credentials provider function, or false.');
        }
    }
    public static function _default_credential_provider(array $args)
    {
        return \WPMailSMTP\Vendor\Aws\Credentials\CredentialProvider::defaultProvider($args);
    }
    public static function _apply_token($value, array &$args)
    {
        ...
        if ($value instanceof \WPMailSMTP\Vendor\Aws\Token\Token) {
            $args['token'] = \WPMailSMTP\Vendor\Aws\Token\TokenProvider::fromToken($value);
        } elseif (\is_array($value) && isset($value['token'])) {
            $args['token'] = \WPMailSMTP\Vendor\Aws\Token\TokenProvider::fromToken(new \WPMailSMTP\Vendor\Aws\Token\Token($value['token'], $value['expires'] ?? null));
        } elseif ($value instanceof \WPMailSMTP\Vendor\Aws\CacheInterface) {
            $args['token'] = \WPMailSMTP\Vendor\Aws\Token\TokenProvider::defaultProvider($args);
        } else {
            throw new \InvalidArgumentException('Token must be an instance of ' . \WPMailSMTP\Vendor\Aws\Token\TokenInterface::class . ', an associative ' . 'array that contains "token" and an optional "expires" ' . 'key-value pairs, a token provider function, or false.');
        }
    }
    public static function _default_token_provider(array $args)
    {
        return \WPMailSMTP\Vendor\Aws\Token\TokenProvider::defaultProvider($args);
    }
    public static function _apply_csm($value, array &$args, \WPMailSMTP\Vendor\Aws\HandlerList $list)
    {
        if ($value === \false) {
            $value = new \WPMailSMTP\Vendor\Aws\ClientSideMonitoring\Configuration(\false, \WPMailSMTP\Vendor\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_HOST, \WPMailSMTP\Vendor\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_PORT, \WPMailSMTP\Vendor\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_CLIENT_ID);
            $args['csm'] = $value;
        }
        $list->appendBuild(\WPMailSMTP\Vendor\Aws\ClientSideMonitoring\ApiCallMonitoringMiddleware::wrap($args['credentials'], $value, $args['region'], $args['api']->getServiceId()), 'ApiCallMonitoringMiddleware');
        $list->appendAttempt(\WPMailSMTP\Vendor\Aws\ClientSideMonitoring\ApiCallAttemptMonitoringMiddleware::wrap($args['credentials'], $value, $args['region'], $args['api']->getServiceId()), 'ApiCallAttemptMonitoringMiddleware');
    }
    public static function _apply_api_provider(callable $value, array &$args)
    {
        $api = new \WPMailSMTP\Vendor\Aws\Api\Service(\WPMailSMTP\Vendor\Aws\Api\ApiProvider::resolve($value, 'api', $args['service'], $args['version']), $value);
        if (empty($args['config']['signing_name']) && isset($api['metadata']['signingName'])) {
            $args['config']['signing_name'] = $api['metadata']['signingName'];
        }
        $args['api'] = $api;
        $args['parser'] = \WPMailSMTP\Vendor\Aws\Api\Service::createParser($api);
        $args['error_parser'] = \WPMailSMTP\Vendor\Aws\Api\Service::createErrorParser($api->getProtocol(), $api);
    }
    public static function _apply_endpoint_provider($value, array &$args)
    {
        if (!isset($args['endpoint'])) {
            if ($value instanceof \WPMailSMTP\Vendor\Aws\EndpointV2\EndpointProviderV2) {
                $options = self::getEndpointProviderOptions($args);
                $value = \WPMailSMTP\Vendor\Aws\Endpoint\PartitionEndpointProvider::defaultProvider($options)->getPartition($args['region'], $args['service']);
            }
            ...
            if (!self::isValidRegion($args['region'])) {
                throw new \WPMailSMTP\Vendor\Aws\Exception\InvalidRegionException('Region must be a valid RFC' . ' host label.');
            }
            ...
            $args['region'] = \WPMailSMTP\Vendor\Aws\strip_fips_pseudo_regions($args['region']);
            // Invoke the endpoint provider and throw if it does not resolve.
            $result = \WPMailSMTP\Vendor\Aws\Endpoint\EndpointProvider::resolve($value, ['service' => $endpointPrefix, 'region' => $args['region'], 'scheme' => $args['scheme'], 'options' => self::getEndpointProviderOptions($args)]);
    ...
    {
        return \WPMailSMTP\Vendor\Aws\EndpointDiscovery\ConfigurationProvider::defaultProvider($args);
    }
    public static function _apply_use_fips_endpoint($value, array &$args)
    {
        if ($value instanceof \WPMailSMTP\Vendor\Aws\CacheInterface) {
            $value = \WPMailSMTP\Vendor\Aws\Endpoint\UseFipsEndpoint\ConfigurationProvider::defaultProvider($args);
        }
        if (\is_callable($value)) {
            $value = $value();
        }
        ...
        if ($value instanceof \WPMailSMTP\Vendor\Aws\Endpoint\UseFipsEndpoint\ConfigurationInterface) {
            $args['config']['use_fips_endpoint'] = $value;
        } else {
            // The Configuration class itself will validate other inputs
            $args['config']['use_fips_endpoint'] = new \WPMailSMTP\Vendor\Aws\Endpoint\UseFipsEndpoint\Configuration($value);
        }
    }
    public static function _default_use_fips_endpoint(array &$args)
    {
        return \WPMailSMTP\Vendor\Aws\Endpoint\UseFipsEndpoint\ConfigurationProvider::defaultProvider($args);
    }
    public static function _apply_use_dual_stack_endpoint($value, array &$args)
    {
        if ($value instanceof \WPMailSMTP\Vendor\Aws\CacheInterface) {
            $value = \WPMailSMTP\Vendor\Aws\Endpoint\UseDualstackEndpoint\ConfigurationProvider::defaultProvider($args);
        }
        if (\is_callable($value)) {
            $value = $value();
        }
        ...
        if ($value instanceof \WPMailSMTP\Vendor\Aws\Endpoint\UseDualstackEndpoint\ConfigurationInterface) {
            $args['config']['use_dual_stack_endpoint'] = $value;
        } else {
            // The Configuration class itself will validate other inputs
            $args['config']['use_dual_stack_endpoint'] = new \WPMailSMTP\Vendor\Aws\Endpoint\UseDualstackEndpoint\Configuration($value, $args['region']);
        }
    }
    public static function _default_use_dual_stack_endpoint(array &$args)
    {
        return \WPMailSMTP\Vendor\Aws\Endpoint\UseDualstackEndpoint\ConfigurationProvider::defaultProvider($args);
    }
    public static function _apply_serializer($value, array &$args, \WPMailSMTP\Vendor\Aws\HandlerList $list)
    {
        $list->prependBuild(\WPMailSMTP\Vendor\Aws\Middleware::requestBuilder($value), 'builder');
    }
    public static function _apply_debug($value, array &$args, \WPMailSMTP\Vendor\Aws\HandlerList $list)
    {
        if ($value !== \false) {
            $list->interpose(new \WPMailSMTP\Vendor\Aws\TraceMiddleware($value === \true ? [] : $value, $args['api']));
        }
    }
    public static function _apply_stats($value, array &$args, \WPMailSMTP\Vendor\Aws\HandlerList $list)
    {
        ...
        if ($args['stats']['timer']) {
            $list->prependInit(\WPMailSMTP\Vendor\Aws\Middleware::timer(), 'timer');
        }
    }
    public static function _apply_profile($_, array &$args)
    {
        $args['credentials'] = \WPMailSMTP\Vendor\Aws\Credentials\CredentialProvider::ini($args['profile']);
    }
    public static function _apply_validate($value, array &$args, \WPMailSMTP\Vendor\Aws\HandlerList $list)
    {
        if ($value === \false) {
            return;
        }
        $validator = $value === \true ? new \WPMailSMTP\Vendor\Aws\Api\Validator() : new \WPMailSMTP\Vendor\Aws\Api\Validator($value);
        $list->appendValidate(\WPMailSMTP\Vendor\Aws\Middleware::validation($args['api'], $validator), 'validation');
    }
    public static function _apply_handler($value, array &$args, \WPMailSMTP\Vendor\Aws\HandlerList $list)
    {
        $list->setHandler($value);
    }
    public static function _default_handler(array &$args)
    {
        return new \WPMailSMTP\Vendor\Aws\WrappedHttpHandler(default_http_handler(), $args['parser'], $args['error_parser'], $args['exception_class'], $args['stats']['http']);
    }
    public static function _apply_http_handler($value, array &$args, \WPMailSMTP\Vendor\Aws\HandlerList $list)
    {
    ...
    public static function _apply_app_id($value, array &$args)
    {
        ...
        if (\strlen($value) > $MAX_APP_ID_LENGTH) {
            \trigger_error("The provided or configured value for `AppId`, " . "which is an user agent parameter, exceeds the maximum length of " . "{$MAX_APP_ID_LENGTH} characters.", \E_USER_WARNING);
        }
        $args['app_id'] = $value;
    }
    ...
    public static function _apply_idempotency_auto_fill($value, array &$args, \WPMailSMTP\Vendor\Aws\HandlerList $list)
    {
        ...
        if ($enabled) {
            $list->prependInit(\WPMailSMTP\Vendor\Aws\IdempotencyTokenMiddleware::wrap($args['api'], $generator), 'idempotency_auto_fill');
        }
    }
    public static function _default_account_id_endpoint_mode($args)
    {
        return \WPMailSMTP\Vendor\Aws\Configuration\ConfigurationResolver::resolve('account_id_endpoint_mode', 'preferred', 'string', $args);
    }
    public static function _apply_account_id_endpoint_mode($value, array &$args)
    {
        ...
        if (!\in_array($value, $accountIdEndpointModes)) {
            throw new \InvalidArgumentException("The value provided for the config account_id_endpoint_mode is invalid." . "Valid values are: " . \implode(", ", $accountIdEndpointModes));
        }
    ...
    public static function _default_endpoint_provider(array $args)
    {
        ...
        if (self::isValidService($serviceName) && self::isValidApiVersion($serviceName, $apiVersion)) {
            $ruleset = \WPMailSMTP\Vendor\Aws\EndpointV2\EndpointDefinitionProvider::getEndpointRuleset($service->getServiceName(), $service->getApiVersion());
            return new \WPMailSMTP\Vendor\Aws\EndpointV2\EndpointProviderV2($ruleset, \WPMailSMTP\Vendor\Aws\EndpointV2\EndpointDefinitionProvider::getPartitions());
        }
        $options = self::getEndpointProviderOptions($args);
        return \WPMailSMTP\Vendor\Aws\Endpoint\PartitionEndpointProvider::defaultProvider($options)->getPartition($args['region'], $args['service']);
    }
    public static function _default_serializer(array $args)
    {
        return \WPMailSMTP\Vendor\Aws\Api\Service::createSerializer($args['api'], $args['endpoint']);
    }
    public static function _default_signature_provider()
    {
        return \WPMailSMTP\Vendor\Aws\Signature\SignatureProvider::defaultProvider();
    }
    public static function _default_auth_scheme_resolver(array $args)
    {
        return new \WPMailSMTP\Vendor\Aws\Auth\AuthSchemeResolver($args['credentials'], $args['token']);
    }
    public static function _default_signature_version(array &$args)
    {
        ...
        $args['__partition_result'] = isset($args['__partition_result']) ? isset($args['__partition_result']) : \call_user_func(\WPMailSMTP\Vendor\Aws\Endpoint\PartitionEndpointProvider::defaultProvider(), ['service' => $args['service'], 'region' => $args['region']]);
    ...
    public static function _apply_suppress_php_deprecation_warning($value, &$args)
    {
        if ($value) {
            ...
            $args['suppress_php_deprecation_warning'] = \WPMailSMTP\Vendor\Aws\boolean_value(\getenv("AWS_SUPPRESS_PHP_DEPRECATION_WARNING"));
        } elseif (!empty($_SERVER["AWS_SUPPRESS_PHP_DEPRECATION_WARNING"])) {
            $args['suppress_php_deprecation_warning'] = \WPMailSMTP\Vendor\Aws\boolean_value($_SERVER["AWS_SUPPRESS_PHP_DEPRECATION_WARNING"]);
    ...
    public static function _default_endpoint(array &$args)
    {
        ...
        $serviceIdentifier = \WPMailSMTP\Vendor\Aws\manifest($args['service'])['serviceIdentifier'];
        $value = \WPMailSMTP\Vendor\Aws\Configuration\ConfigurationResolver::resolve('endpoint_url_' . $serviceIdentifier, '', 'string', $args + ['ini_resolver_options' => ['section' => 'services', 'subsection' => $serviceIdentifier, 'key' => 'endpoint_url']]);
        if (empty($value)) {
            $value = \WPMailSMTP\Vendor\Aws\Configuration\ConfigurationResolver::resolve('endpoint_url', '', 'string', $args);
        }
        if (!empty($value)) {
...
EOT;
    ...
    private static function _resolve_from_env_ini(string $key, string $expectedType, array $args)
    {
        ...
        return \WPMailSMTP\Vendor\Aws\Configuration\ConfigurationResolver::resolve($key, $typeDefaultMap[$expectedType] ?? '', $expectedType, $args);
    }
    /**
    ...
    private static function isValidService($service)
    {
        ...
        $services = \WPMailSMTP\Vendor\Aws\manifest();
