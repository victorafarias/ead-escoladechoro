...
<div class="analytics-course-details">
	...
	if ( 'today' === $time_period ) {
		...
		$content_title = sprintf(
			// translators: %s: month.
			__( 'for this month (%s)', 'tutor-pro' ),
			$month
		);
	} elseif ( 'yearly' === $time_period ) {
		$year = gmdate( 'Y' ); //phpcs:ignore

		$content_title = sprintf(
			// translators: %s: year.
			__( 'for this year (%s)', 'tutor-pro' ),
			$year
		);
	}
		$graph_tabs = array(
