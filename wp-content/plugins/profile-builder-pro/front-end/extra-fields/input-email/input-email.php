...
/* handle field output */
function wppb_input_email_handler( $output, $form_location, $field, $user_id, $field_check_errors, $request_data ){
	if ( $field['field'] == 'Email' ){
		...
		if ( $form_location != 'back_end' ){
            ...
            $output = '
				...
				<input class="extra_field_input '. apply_filters( 'wppb_fields_extra_css_class', '', $field ) .'" name="'.$field['meta-name'].'" maxlength="'. apply_filters( 'wppb_maximum_character_length', 70, $field ) .'" type="email" id="'.$field['meta-name'].'" value="'. esc_attr( wp_unslash( $input_value ) ) .'" '. $extra_attr .'/>';
            ...
            $output = '
				<table class="form-table">
					<tr>
						...
						<td>
							<input class="custom_field_input" size="45" name="'.$field['meta-name'].'" maxlength="'. apply_filters( 'wppb_maximum_character_length', 70, $field ) .'" type="email" id="'.$field['meta-name'].'" value="'. esc_attr( $input_value ) .'" '. $extra_attr .'/>
...
add_filter( 'wppb_output_form_field_email', 'wppb_input_email_handler', 10, 6 );
add_filter( 'wppb_admin_output_form_field_email', 'wppb_input_email_handler', 10, 6 );

/* handle field save */
function wppb_save_input_email_value( $field, $user_id, $request_data, $form_location ){
	if( $field['field'] == 'Email' ){
		if ( isset( $request_data[wppb_handle_meta_name( $field['meta-name'] )] ) )
			update_user_meta( $user_id, $field['meta-name'], $request_data[wppb_handle_meta_name( $field['meta-name'] )] );
	}
}
add_action( 'wppb_save_form_field', 'wppb_save_input_email_value', 10, 4 );
add_action( 'wppb_backend_save_form_field', 'wppb_save_input_email_value', 10, 4 );

/* handle field validation */
function wppb_check_input_email_value( $message, $field, $request_data, $form_location ){
	if( $field['field'] == 'Email' ){
        ...
        if ( !empty( $request_data[wppb_handle_meta_name( $field['meta-name'] )] ) ) {
            if ( !preg_match( '/^[a-zA-Z0-9.!#$%&\'*+\/=?^_`{|}~-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)*$/', $request_data[wppb_handle_meta_name( $field['meta-name'] )] ) ) {
...
add_filter( 'wppb_check_form_field_email', 'wppb_check_input_email_value', 10, 4 );
