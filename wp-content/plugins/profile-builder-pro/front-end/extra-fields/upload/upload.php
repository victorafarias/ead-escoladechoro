...
function wppb_upload_handler( $output, $form_location, $field, $user_id, $field_check_errors, $request_data ){
    if ( $field['field'] == 'Upload' ){
        ...
        if( $form_location != 'register' ) {
            ...
            if( !empty( $input_value ) && !is_numeric( $input_value ) && apply_filters( 'wppb_upload_field_transform_file_to_attachment', true, $field ) ){
...
function wppb_save_upload_value( $field, $user_id, $request_data, $form_location ){
    if( $field[ 'field' ] == 'Upload' ){
        ...
        /**
         * Fires after the user meta-data has been updated
         *
         * @param int    $user_id - the ID of the User
         * @param string $request_data[wppb_handle_meta_name( $field['meta-name'] )] - the Attachment ID (also the Attachment-Post ID)
         * @param string $field['meta-name'] - the Upload Field meta-name
         * @param string $field['simple-upload'] - "yes" if the field is a Simple Upload field, empty otherwise
         *
         */
        do_action( 'wppb_upload_field_after_user_meta_update', $user_id, $request_data[wppb_handle_meta_name( $field['meta-name'] )], $field['meta-name'], $field['simple-upload'] );
