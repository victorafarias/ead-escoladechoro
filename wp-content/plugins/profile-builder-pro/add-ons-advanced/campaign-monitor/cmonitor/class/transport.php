...
define('WPPB_IN_CS_REST_GET', 'GET');
define('WPPB_IN_CS_REST_POST', 'POST');
define('WPPB_IN_CS_REST_PUT', 'PUT');
define('WPPB_IN_CS_REST_DELETE', 'DELETE');
if (false === defined('WPPB_IN_CS_REST_SOCKET_TIMEOUT')) {
    define('WPPB_IN_CS_REST_SOCKET_TIMEOUT', 10);
}
if (false === defined('WPPB_IN_CS_REST_CALL_TIMEOUT')) {
    define('WPPB_IN_CS_REST_CALL_TIMEOUT', 10);
}

function WPPB_IN_CS_REST_TRANSPORT_get_available($requires_ssl, $log) {
    if(function_exists('curl_init') && function_exists('curl_exec')) {
        return new WPPB_IN_CS_REST_CurlTransport($log);
    } else if(WPPB_IN_CS_REST_TRANSPORT_can_use_raw_socket($requires_ssl)) {
        return new WPPB_IN_CS_REST_SocketTransport($log);
    } else { 
        $log->log_message('No transport is available', __FUNCTION__, WPPB_IN_CS_REST_LOG_ERROR);
...
function WPPB_IN_CS_REST_TRANSPORT_can_use_raw_socket($requires_ssl) {
...
class WPPB_IN_CS_REST_BaseTransport {
    ...
    function split_and_inflate($response, $may_be_compressed) {        
        ...
        if($may_be_compressed && preg_match('/^Content-Encoding:\s+gzip\s+$/im', $headers)) {        
            ...
            $this->_log->log_message('Inflated gzipped response: '.$original_length.' bytes ->'.
                strlen($result).' bytes', get_class(), WPPB_IN_CS_REST_LOG_VERBOSE);
        }
 ...
 * @since 1.0
 */
class WPPB_IN_CS_REST_CurlTransport extends WPPB_IN_CS_REST_BaseTransport {
    ...
    function make_call($call_options) {
        ...
        curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, WPPB_IN_CS_REST_SOCKET_TIMEOUT);
        curl_setopt($ch, CURLOPT_TIMEOUT, WPPB_IN_CS_REST_CALL_TIMEOUT);
        ...
        switch($call_options['method']) {
            case WPPB_IN_CS_REST_PUT:
                curl_setopt($ch, CURLOPT_CUSTOMREQUEST, WPPB_IN_CS_REST_PUT);
                $headers[] = 'Content-Length: '.strlen($call_options['data']);
                curl_setopt($ch, CURLOPT_POSTFIELDS, $call_options['data']);
                break;
            case WPPB_IN_CS_REST_POST:
                curl_setopt($ch, CURLOPT_POST, true);
                curl_setopt($ch, CURLOPT_POSTFIELDS, isset($call_options['data']) ? $call_options['data'] : '');
                break;
            case WPPB_IN_CS_REST_DELETE:
                curl_setopt($ch, CURLOPT_CUSTOMREQUEST, WPPB_IN_CS_REST_DELETE);
                break;
        }
        ...
        if(!$response && $response !== '') {
            $this->_log->log_message('Error making request with curl_error: '.curl_errno($ch),
                get_class($this), WPPB_IN_CS_REST_LOG_ERROR);
            trigger_error('Error making request with curl_error: '.curl_error($ch), E_USER_ERROR); //phpcs:ignore
        }
...
class WPPB_IN_CS_REST_SocketWrapper {
    var $socket;

    function open($domain, $port) {
        $this->socket = fsockopen($domain, $port, $errno, $errstr, WPPB_IN_CS_REST_SOCKET_TIMEOUT);

        if(!$this->socket) {
            die('Error making request with '.$errno.': '.$errstr);  //phpcs:ignore
            return false;
        } else if(function_exists('stream_set_timeout')) {
            stream_set_timeout($this->socket, WPPB_IN_CS_REST_SOCKET_TIMEOUT);
        }
...
class WPPB_IN_CS_REST_SocketTransport extends WPPB_IN_CS_REST_BaseTransport {
    ...
    function __construct($log, $socket_wrapper = NULL) {
        ...
        if(is_null($socket_wrapper)) {
            $socket_wrapper = new WPPB_IN_CS_REST_SocketWrapper();
        }
