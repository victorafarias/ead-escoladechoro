...
namespace ProfileBuilder\BuddyPressAddon;
 ...
 *
 * "WPPB_BDP_" prefix was appended to all the define constants.
 * namespace TranslatePress was added.
 * MAX_FILE_SIZE has been modified from value 600000 to new value: define( 'WPPB_BDP_MAX_FILE_SIZE', 100000000 );
 *
 */

define('WPPB_BDP_HDOM_TYPE_ELEMENT', 1);
define('WPPB_BDP_HDOM_TYPE_COMMENT', 2);
define('WPPB_BDP_HDOM_TYPE_TEXT',	3);
define('WPPB_BDP_HDOM_TYPE_ENDTAG',  4);
define('WPPB_BDP_HDOM_TYPE_ROOT',	5);
define('WPPB_BDP_HDOM_TYPE_UNKNOWN', 6);
define('WPPB_BDP_HDOM_QUOTE_DOUBLE', 0);
define('WPPB_BDP_HDOM_QUOTE_SINGLE', 1);
define('WPPB_BDP_HDOM_QUOTE_NO',	 3);
define('WPPB_BDP_HDOM_INFO_BEGIN',   0);
define('WPPB_BDP_HDOM_INFO_END',	 1);
define('WPPB_BDP_HDOM_INFO_QUOTE',   2);
define('WPPB_BDP_HDOM_INFO_SPACE',   3);
define('WPPB_BDP_HDOM_INFO_TEXT',	4);
define('WPPB_BDP_HDOM_INFO_INNER',   5);
define('WPPB_BDP_HDOM_INFO_OUTER',   6);
define('WPPB_BDP_HDOM_INFO_ENDSPACE',7);
define('WPPB_BDP_DEFAULT_TARGET_CHARSET', 'UTF-8');
define('WPPB_BDP_DEFAULT_BR_TEXT', "\r\n");
define('WPPB_BDP_DEFAULT_SPAN_TEXT', " ");
define( 'WPPB_BDP_MAX_FILE_SIZE', 100000000 );

/** Contents between curly braces "{" and "}" are interpreted as text */
define('WPPB_BDP_HDOM_SMARTY_AS_TEXT', 1);

// helper functions
// -----------------------------------------------------------------------------
// get html dom from file
// $maxlen is defined in the code as PHP_STREAM_COPY_ALL which is defined as -1.
function file_get_html($url, $use_include_path = false, $context=null, $offset = 0, $maxLen=-1, $lowercase = true, $forceTagsClosed=true, $target_charset = WPPB_BDP_DEFAULT_TARGET_CHARSET, $stripRN=true, $defaultBRText=WPPB_BDP_DEFAULT_BR_TEXT, $defaultSpanText=WPPB_BDP_DEFAULT_SPAN_TEXT)
{
	// Ensure maximum length is greater than zero
	if($maxLen <= 0) { $maxLen = WPPB_BDP_MAX_FILE_SIZE; }
...
function str_get_html($str, $lowercase=true, $forceTagsClosed=true, $target_charset = WPPB_BDP_DEFAULT_TARGET_CHARSET, $stripRN=true, $defaultBRText=WPPB_BDP_DEFAULT_BR_TEXT, $defaultSpanText=WPPB_BDP_DEFAULT_SPAN_TEXT)
{
	$dom = new simple_html_dom(null, $lowercase, $forceTagsClosed, $target_charset, $stripRN, $defaultBRText, $defaultSpanText);
	if (empty($str) || strlen($str) > WPPB_BDP_MAX_FILE_SIZE)
	{
		$dom->clear();
		return false;
	}
...
class simple_html_dom_node
{
	 ...
	 * Node type
	 *
	 * Default is {@see WPPB_BDP_HDOM_TYPE_TEXT}
	 *
	 * @var int
	 */
	public $nodetype = WPPB_BDP_HDOM_TYPE_TEXT;
	...
	// The "info" array - see WPPB_BDP_HDOM_INFO_... for what each element contains.
	public $_ = array();
	...
	function dump($show_attr=true, $deep=0)
	{
		...
		echo $lead.$this->tag;  //phpcs:ignore
		if ($show_attr && count($this->attr)>0)
		{
			echo '(';
			foreach ($this->attr as $k=>$v)
				echo "[$k]=>\"".$this->$k.'", '; //phpcs:ignore
			echo ')';
		}
		echo "\n";
	...
	function dump_node($echo=true)
	{
		...
		$string .= " WPPB_BDP_HDOM_INNER_INFO: '";
		if (isset($node->_[WPPB_BDP_HDOM_INFO_INNER]))
		{
			$string .= $node->_[WPPB_BDP_HDOM_INFO_INNER] . "'";
		}
		else
		{
			$string .= ' NULL ';
	...
	function innertext()
	{
		if (isset($this->_[WPPB_BDP_HDOM_INFO_INNER])) return $this->_[WPPB_BDP_HDOM_INFO_INNER];
		if (isset($this->_[WPPB_BDP_HDOM_INFO_TEXT])) return $this->dom->restore_noise($this->_[WPPB_BDP_HDOM_INFO_TEXT]);
	...
	function outertext()
	{
		...
		if (isset($this->_[WPPB_BDP_HDOM_INFO_OUTER])) return $this->_[WPPB_BDP_HDOM_INFO_OUTER];
		if (isset($this->_[WPPB_BDP_HDOM_INFO_TEXT])) return $this->dom->restore_noise($this->_[WPPB_BDP_HDOM_INFO_TEXT]);

		// render begin tag
		if ($this->dom && $this->dom->nodes[$this->_[WPPB_BDP_HDOM_INFO_BEGIN]])
		{
			$ret = $this->dom->nodes[$this->_[WPPB_BDP_HDOM_INFO_BEGIN]]->makeup();
		} else {
			$ret = "";
		}

		// render inner text
		if (isset($this->_[WPPB_BDP_HDOM_INFO_INNER]))
		{
			// If it's a br tag...  don't return the WPPB_BDP_HDOM_INNER_INFO that we may or may not have added.
			if ($this->tag != "br")
			{
				$ret .= $this->_[WPPB_BDP_HDOM_INFO_INNER];
			}
		} else {
			if ($this->nodes)
			{
		...
		// render end tag
		if (isset($this->_[WPPB_BDP_HDOM_INFO_END]) && $this->_[WPPB_BDP_HDOM_INFO_END]!=0)
			$ret .= '</'.$this->tag.'>';
		return $ret;
	}
	...
	function text()
	{
		...
		{
			case WPPB_BDP_HDOM_TYPE_TEXT: return $this->dom->restore_noise($this->_[WPPB_BDP_HDOM_INFO_TEXT]);
			case WPPB_BDP_HDOM_TYPE_COMMENT: return '';
			case WPPB_BDP_HDOM_TYPE_UNKNOWN: return '';
		}
		if (strcasecmp($this->tag, 'script')===0) return '';
		if (strcasecmp($this->tag, 'style')===0) return '';

		$ret = '';
		// In rare cases, (always node type 1 or WPPB_BDP_HDOM_TYPE_ELEMENT - observed for some span tags, and some p tags) $this->nodes is set to NULL.
	...
	function makeup()
	{
		...
		foreach ($this->attr as $key=>$val)
		{
			...
			$ret .= $this->_[WPPB_BDP_HDOM_INFO_SPACE][$i][0];
			//no value attr: nowrap, checked selected...
			if ($val===true)
				$ret .= $key;
			else {
				switch ($this->_[WPPB_BDP_HDOM_INFO_QUOTE][$i])
				{
					case WPPB_BDP_HDOM_QUOTE_DOUBLE: $quote = '"'; break;
					case WPPB_BDP_HDOM_QUOTE_SINGLE: $quote = '\''; break;
					default: $quote = '';
				}
				$ret .= $key.$this->_[WPPB_BDP_HDOM_INFO_SPACE][$i][1].'='.$this->_[WPPB_BDP_HDOM_INFO_SPACE][$i][2].$quote.$val.$quote;
			}
		}
		$ret = $this->dom->restore_noise($ret);
		return $ret . $this->_[WPPB_BDP_HDOM_INFO_ENDSPACE] . '>';
	}
	...
	function find($selector, $idx=null, $lowercase=false)
	{
		...
		for ($c=0; $c<$count; ++$c)
		{
			...
			if (!isset($this->_[WPPB_BDP_HDOM_INFO_BEGIN])) return array();

			$head = array($this->_[WPPB_BDP_HDOM_INFO_BEGIN]=>1);
	...
	protected function seek($selector, &$ret, $lowercase=false)
	{
		...
		if ($tag && $key && is_numeric($key))
		{
			...
			foreach ($this->children as $c)
			{
				if ($tag==='*' || $tag===$c->tag) {
					if (++$count==$key) {
						$ret[$c->_[WPPB_BDP_HDOM_INFO_BEGIN]] = 1;
						return;
					}
				}
			}
		...
		$end = (!empty($this->_[WPPB_BDP_HDOM_INFO_END])) ? $this->_[WPPB_BDP_HDOM_INFO_END] : 0;
		if ($end==0) {
			$parent = $this->parent;
			while (!isset($parent->_[WPPB_BDP_HDOM_INFO_END]) && $parent!==null) {
				$end -= 1;
				$parent = $parent->parent;
			}
			$end += $parent->_[WPPB_BDP_HDOM_INFO_END];
		}

		for ($i=$this->_[WPPB_BDP_HDOM_INFO_BEGIN]+1; $i<$end; ++$i) {
...
//		$pattern = "/([\w-:\*]*)(?:\#([\w-]+)|\.([\w-]+))?(?:\[@?(!?[\w-]+)(?:([!*^$]?=)[\"']?(.*?)[\"']?)?\])?([\/, ]+)/is";
	...
	function __set($name, $value)
	{
		...
		switch ($name)
		{
			case 'outertext': return $this->_[WPPB_BDP_HDOM_INFO_OUTER] = $value;
			case 'innertext':
				if (isset($this->_[WPPB_BDP_HDOM_INFO_TEXT])) return $this->_[WPPB_BDP_HDOM_INFO_TEXT] = $value;
				return $this->_[WPPB_BDP_HDOM_INFO_INNER] = $value;
		}
		if (!isset($this->attr[$name]))
		{
			$this->_[WPPB_BDP_HDOM_INFO_SPACE][] = array(' ', '', '');
			$this->_[WPPB_BDP_HDOM_INFO_QUOTE][] = WPPB_BDP_HDOM_QUOTE_DOUBLE;
		}
...
class simple_html_dom
{
	...
	function __construct($str=null, $lowercase=true, $forceTagsClosed=true, $target_charset=WPPB_BDP_DEFAULT_TARGET_CHARSET, $stripRN=true, $defaultBRText=WPPB_BDP_DEFAULT_BR_TEXT, $defaultSpanText=WPPB_BDP_DEFAULT_SPAN_TEXT, $options=0)
	{
		if ($str)
		{
	...
	function load($str, $lowercase=true, $stripRN=true, $defaultBRText=WPPB_BDP_DEFAULT_BR_TEXT, $defaultSpanText=WPPB_BDP_DEFAULT_SPAN_TEXT, $options=0)
	{
		global $debug_object;
		...
		if($options & WPPB_BDP_HDOM_SMARTY_AS_TEXT) { // Strip Smarty scripts
		...
		$this->parse();
		// end
		$this->root->_[WPPB_BDP_HDOM_INFO_END] = $this->cursor;
		$this->parse_charset();
	...
	protected function prepare($str, $lowercase=true, $defaultBRText=WPPB_BDP_DEFAULT_BR_TEXT, $defaultSpanText=WPPB_BDP_DEFAULT_SPAN_TEXT)
	{
		$this->clear();
		...
		$this->root->tag = 'root';
		$this->root->_[WPPB_BDP_HDOM_INFO_BEGIN] = -1;
		$this->root->nodetype = WPPB_BDP_HDOM_TYPE_ROOT;
	...
	protected function parse()
	{
		while (true) {
			...
			++$this->cursor;
			$node->_[WPPB_BDP_HDOM_INFO_TEXT] = $s;
	...
	protected function read_tag()
	{
		...
		if ($this->char==='/')
		{
			...
			if ($parent_lower!==$tag_lower)
			{
				...
				{
					$this->parent->_[WPPB_BDP_HDOM_INFO_END] = 0;
					...
					if (strtolower($this->parent->tag)!==$tag_lower) {
						...
						$this->parent->_[WPPB_BDP_HDOM_INFO_END] = $this->cursor;
				...
				{
					$this->parent->_[WPPB_BDP_HDOM_INFO_END] = 0; // No end tag
					$org_parent = $this->parent;
		...
		// start tag
		$node = new simple_html_dom_node($this);
		$node->_[WPPB_BDP_HDOM_INFO_BEGIN] = $this->cursor;
		++$this->cursor;
		...
		if (isset($tag[0]) && $tag[0]==='!') {
			$node->_[WPPB_BDP_HDOM_INFO_TEXT] = '<' . $tag . $this->copy_until_char('>');

			if (isset($tag[2]) && $tag[1]==='-' && $tag[2]==='-') { // Comment ("<!--")
				$node->nodetype = WPPB_BDP_HDOM_TYPE_COMMENT;
				$node->tag = 'comment';
			} else { // Could be doctype or CDATA but we don't care
				$node->nodetype = WPPB_BDP_HDOM_TYPE_UNKNOWN;
				$node->tag = 'unknown';
			}
			if ($this->char==='>') $node->_[WPPB_BDP_HDOM_INFO_TEXT].='>';
		...
		if ($pos=strpos($tag, '<')!==false) {
			$tag = '<' . substr($tag, 0, -1);
			$node->_[WPPB_BDP_HDOM_INFO_TEXT] = $tag;
			$this->link_nodes($node, false);
		...
		if (!preg_match("/^\w[\w:-]*$/", $tag)) {
			$node->_[WPPB_BDP_HDOM_INFO_TEXT] = '<' . $tag . $this->copy_until('<>');
		...
		$node->nodetype = WPPB_BDP_HDOM_TYPE_ELEMENT;
		$tag_lower = strtolower($tag);
		...
		do
		{
			...
			if ($this->pos>=$this->size-1 && $this->char!=='>') { // Out of bounds before the tag ended
				$node->nodetype = WPPB_BDP_HDOM_TYPE_TEXT;
				$node->_[WPPB_BDP_HDOM_INFO_END] = 0;
				$node->_[WPPB_BDP_HDOM_INFO_TEXT] = '<'.$tag . $space[0] . $name;
				$node->tag = 'text';
				$this->link_nodes($node, false);
				return true;
			}
			...
			if ($this->doc[$this->pos-1]=='<') { // Attributes cannot start after opening tag
				...
				$node->attr = array();
				$node->_[WPPB_BDP_HDOM_INFO_END] = 0;
				$node->_[WPPB_BDP_HDOM_INFO_TEXT] = substr($this->doc, $begin_tag_pos, $this->pos-$begin_tag_pos-1);
				$this->pos -= 2;
				$this->char = (++$this->pos<$this->size) ? $this->doc[$this->pos] : null; // next
				$this->link_nodes($node, false);
				return true;
			...
			if ($name!=='/' && $name!=='') { // this is a attribute name
				...
				if ($this->char==='=') { // attribute with value
					...
					//no value attr: nowrap, checked selected...
					$node->_[WPPB_BDP_HDOM_INFO_QUOTE][] = WPPB_BDP_HDOM_QUOTE_NO;
					$node->attr[$name] = true;
					if ($this->char!='>') $this->char = $this->doc[--$this->pos]; // prev
				}
				$node->_[WPPB_BDP_HDOM_INFO_SPACE][] = $space;
		...
		$this->link_nodes($node, true);
		$node->_[WPPB_BDP_HDOM_INFO_ENDSPACE] = $space[0];

		// handle empty tags (i.e. "<div/>")
		if ($this->copy_until_char('>')==='/')
		{
			$node->_[WPPB_BDP_HDOM_INFO_ENDSPACE] .= '/';
			$node->_[WPPB_BDP_HDOM_INFO_END] = 0;
		}
		else
		{
		...
		if ($node->tag == "br")
		{
			$node->_[WPPB_BDP_HDOM_INFO_INNER] = $this->default_br_text;
		}
	...
	protected function parse_attr($node, $name, &$space)
	{
		...
		switch ($this->char) {
			case '"': // value is anything between double quotes
				$node->_[WPPB_BDP_HDOM_INFO_QUOTE][] = WPPB_BDP_HDOM_QUOTE_DOUBLE;
				$this->char = (++$this->pos<$this->size) ? $this->doc[$this->pos] : null; // next
				$node->attr[$name] = $this->restore_noise($this->copy_until_char('"'));
				$this->char = (++$this->pos<$this->size) ? $this->doc[$this->pos] : null; // next
				break;
			case '\'': // value is anything between single quotes
				$node->_[WPPB_BDP_HDOM_INFO_QUOTE][] = WPPB_BDP_HDOM_QUOTE_SINGLE;
				$this->char = (++$this->pos<$this->size) ? $this->doc[$this->pos] : null; // next
				$node->attr[$name] = $this->restore_noise($this->copy_until_char('\''));
				$this->char = (++$this->pos<$this->size) ? $this->doc[$this->pos] : null; // next
				break;
