...
function wppb_in_woo_get_billing_fields(){
    return array(
        ...
        'billing_address_2' => array('label' => 'Address 2', 'required' => 'No'),
...
function wppb_in_woo_billing_handler( $output, $form_location, $field, $user_id, $field_check_errors, $request_data ){
    if ( $field['field'] == 'WooCommerce Customer Billing Address' ){
        ...
        * */
        ...
        // Display "Ship to a different address" checkbox if we're on the PB Register or Edit Profile forms and Woo Shipping Address is added under Manage Fields
        if ( ( $form_location == 'register' || $form_location == 'edit_profile' ) && $in_manage_fields ) {
            ...
            $checked = 'checked';
            if ( $form_location != 'register' ){
                $different_shipping_address = get_user_meta($user_id, 'woo_different_shipping_address', true);
                if ( !empty($different_shipping_address) && ($different_shipping_address == 'no') ) {
                    $checked = '';
...
function wppb_in_save_woo_billing_value( $field, $user_id, $request_data, $form_location ){
    if( $field['field'] == 'WooCommerce Customer Billing Address' ) {
        ...
        if (isset($request_data['woo_different_shipping_address'])) {
            update_user_meta($user_id, 'woo_different_shipping_address', $request_data['woo_different_shipping_address']);
        }
    }
}
