...
class LP_Gateway_Woo extends LP_Gateway_Abstract {
	...
	public function __construct() {
		...
		$this->method_title       = $this->title = __( 'WooCommerce Payment', 'ecademy-toolkit' );
		$this->method_description = __( 'Make a payment with WooCommerce payment methods.', 'ecademy-toolkit' );
	...
	public function get_settings() {
		...
		$lpw_default = $available_payment_html ? sprintf( '<ul class="learn-press-woo-payments">%s</ul>',
			$available_payment_html ) : '';
		$lpw_html    = '';
		$lpw_type    = 'checkbox';
		...
		return
			array(
				...
				array(
					'title'   => __( 'Enable', 'ecademy-toolkit' ),
				...
				array(
					'title'   => esc_html__( 'Redirect to Woo checkout', 'ecademy-toolkit' ),
					'id'      => 'redirect_to_checkout',
					'default' => 'no',
					'type'    => $lpw_type,
					'class'   => '',
					'desc'    => __( 'Enable to redirect to page Checkout when add to cart' . $desc_guest_checkout, 'ecademy-toolkit' ),
				),
				array(
					'title'   => esc_html__( 'WooCommerce Payments', 'ecademy-toolkit' ),
	...
	public function is_available( $available, $gateway ) {
		return ECA_Addon_Woo_Payment::instance()->is_enabled() && ECA_Addon_Woo_Payment::instance()->woo_payment_enabled() && sizeof( WC()->payment_gateways()->get_available_payment_gateways() );
	}
