...
/**
 * Class ECA_Addon_Woo_Payment
 */
class ECA_Addon_Woo_Payment extends LP_Addon {
	/**
	 * @var string
	 */
	public $version = ECA_ADDON_WOO_PAYMENT_VER;

	/**
	 * @var string
	 */
	public $require_version = ECA_ADDON_WOO_PAYMENT_REQUIRE_VER;

	/**
	 * @var string
	 */
	public $plugin_file = ECA_ADDON_WOO_PAYMENT_FILE;
	...
	/**
	 * @var ECA_Addon_Woo_Payment|null
	 *
...
//		add_filter( 'template_include', array( $this, 'learn_press_wc_defines' ) );
		...
		<div class="notice notice-warning woo-payment-order-notice">
			<p>
				<?php echo sprintf( '%s <a href="%s">#%s</a>',
					__( 'This order is related to LearnPress order', 'ecademy-toolkit' ),
	...
	public function create_order( $wc_order_id, $posted ) {
		...
		$order_data = array(
			...
			'ping_status' => 'closed',
			'post_title'  => __( 'Order on', 'ecademy-toolkit' ) . ' ' . current_time( "l jS F Y h:i:s A" ),
			'meta_input'  => array(
	...
	public function custom_add_to_cart_message( $message ) {
		if ( $this->_response['single_purchase'] ) {
			if ( preg_match( '~<a.*>(.*)</a>~', $message, $m ) ) {
				$link    = preg_replace( '~>(.*)<~', '>' . __( 'Checkout', 'ecademy-toolkit' ) . '<', $m[0] );
	...
	public function load_assets() {
		$version_asset = ECA_ADDON_WOO_PAYMENT_VER;
		...
		wp_register_style(
			'lp-woo-css',
			plugins_url( '/', ECA_ADDON_WOO_PAYMENT_FILE ) . 'assets/lp_woo' . LP_Assets::$_min_assets . '.css',
			array(),
			$version_asset
		);
		wp_register_script(
			'lp-woo-payment-js',
			plugins_url( '/', ECA_ADDON_WOO_PAYMENT_FILE ) . 'assets/lp_woo' . LP_Assets::$_min_assets . '.js',
			array(),
			$version_asset,
			true
		);
		...
		switch ( self::$_error ) {
			case 1:
				...
				echo '<p>' . sprintf( __( '<strong>WooCommerce Payment Gateways</strong> addon for <strong>LearnPress</strong> requires <a href="%s">WooCommerce</a> plugin is installed.',
						'ecademy-toolkit' ), 'http://wordpress.org/plugins/woocommerce' ) . '</p>';
				echo '</div>';
				break;
			case 2:
				?>
				<div class="error">
					<p><?php printf( __( '<strong>WooCommerce</strong> addon version %s requires <strong>LearnPress</strong> version %s or higher',
							'ecademy-toolkit' ), ECA_ADDON_WOO_PAYMENT_VER,
							ECA_ADDON_WOO_PAYMENT_REQUIRE_VER ); ?></p>
				</div>
				<?php
				break;
			case 3:
