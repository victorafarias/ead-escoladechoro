...
class LP_Woo_Ajax {
	...
	public function lpWooAddCourseToCart() {
		$result = array( 'code' => 0, 'message' => __( 'error', 'ecademy-toolkit' ) );
		...
		if ( ! isset( $_POST['course-id'] ) || ! isset( $_POST['add-course-to-cart-nonce'] )
			...
			$result['message'] = __( 'Params invalid', 'ecademy-toolkit' );
		...
		if ( $course ) {
			$result['message'] = __( 'course is invalid', 'ecademy-toolkit' );
		}
		...
		if ( $cart_item_key ) {
			...
			$result['button_view_cart'] = '<a class="btn-lp-course-view-cart" href="' . wc_get_cart_url() . '"><button class="lp-button">' . __( 'View cart', 'ecademy-toolkit' ) . '</button></a>';
			...
			if ( isset( $wc_notices['error'] ) && ! empty( $wc_notices['error'] ) ) {
				...
				$result['message'] = __( 'You cannot add this course to your cart. Maybe certificate exists', 'ecademy-toolkit' );
			}
		}
