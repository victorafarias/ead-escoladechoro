...
class eCademy_RT {
	...
	function admin_error() {
		$out = '<div class="notice notice-error ecademy-purchase-notice"><p>' . sprintf( wp_kses_post( __( 'The %s theme needs to be registered. %sRegister Now%s', 'ecademy' ) ), 'eCademy', '<a href="' . admin_url( 'admin.php?page=ecademy') . '">' , '</a>' ) . '</p></div>';
		echo wp_kses_post($out);
	}

	function admin_notice() {
		$out = '<div class="notice ecademy-purchase-notice"><p>' .sprintf( wp_kses_post( __( 'Purchase key is invalid. Need a license? %sPurchase Now%s', 'ecademy' ) ), '<a target="_blank" href="https://1.envato.market/KM13e">', '</a>' ) .'</p></div>';
		echo wp_kses_post($out);
	}
	...
	function form() {
		...
		<form action="" method="post" id="verify-envato-purchase" class="et-theme-register-form">
			...
			<input id="ecademy_purchase_valid_code" name="ecademy_purchase_valid_code" type="text" value="<?php echo esc_attr( $license ); ?>" placeholder="<?php esc_attr_e( 'Enter purchase key', 'ecademy' ); ?>">
			...
			if ( $status != '' ) {
				if( $status == 'valid' ){ ?>
					<input id="ecademy_purchase_valid_code" name="ecademy_purchase_valid_code" type="hidden" value="">
		...
		<?php
		if ( isset($_POST['ecademy_purchase_valid_code']) ) {
			if( $_POST['ecademy_purchase_valid_code'] != '' ){
				echo "<meta http-equiv='refresh' content='0'>";
				update_option( $this->theme_slug . '_purchase_valid_code', $_POST['ecademy_purchase_valid_code'] );
				$purchase_code = htmlspecialchars($_POST['ecademy_purchase_valid_code']);
