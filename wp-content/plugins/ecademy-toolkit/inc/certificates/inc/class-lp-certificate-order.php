...
class LP_Certificate_Order {
	...
	protected function __construct() {
		...
		add_filter( 'learn-press/purchase/item-types/can-purchase', array( $this, 'lp_cert_add_item_can_purchase' ), 10, 1 );
	...
	public function lp_cert_order_add_item( $item ) {
		if ( get_post_type( $item['item_id'] ) == LP_ADDON_CERTIFICATES_CERT_CPT ) {
			...
			$item['total']           = $subtotal;
			$item['order_item_name'] = sprintf( '%s %s', __( 'Certificate:', 'ecademy-toolkit' ), get_the_title( $item['item_id'] ) );
		}
		return $item;
	}
	...
	public function lp_order_status_change_to_completed( $lp_order_id ) {
		...
		foreach ( $items as $item ) {
			...
			if ( $cert->post_type == 'lp_cert' ) {
				...
				$query = $wpdb->prepare( "
					DELETE FROM $lp_db->tb_lp_order_itemmeta
	...
	// $order_item_title = sprintf( '%s %s', __( 'Certificate:', 'ecademy-toolkit' ), get_the_title( $cart_item['_learnpress_certificate_id'] ) );
	// }
	...
	public function lp_order_cert_item_link_not_course_id( $link, $item ) {
		...
		if ( isset( $item['_lp_cert_id'] ) && isset( $item['_lp_course_id_of_cert'] ) ) {
			...
			$title = sprintf( '%s: %s - %s', __( 'Certificate', 'ecademy-toolkit' ), $cert_title, $course_title );
		...
		if ( isset( $item['learnpress_certificate_bought'] ) && get_post_type( $item['learnpress_certificate_bought'] ) === LP_ADDON_CERTIFICATES_CERT_CPT ) {
			...
			$title          = sprintf( '%s: %s', __( 'Certificate', 'ecademy-toolkit' ), get_the_title( $item['learnpress_certificate_bought'] ) );
			$link           = '<a href="' . $edit_post_link . '">' . $title . '</a>';
		}
	...
	public function lp_order_cert_item_link( $link, $item ) {
		...
		if ( isset( $item['learnpress_certificate_bought'] ) && get_post_type( $item['learnpress_certificate_bought'] ) === LP_ADDON_CERTIFICATES_CERT_CPT ) {
			$edit_post_link = get_edit_post_link( $item['learnpress_certificate_bought'] );
			$title          = sprintf( '%s %s - %s %s', __( 'Certificate:', 'ecademy-toolkit' ), get_the_title( $item['learnpress_certificate_bought'] ), __( 'Course:', 'ecademy-toolkit' ), get_the_title( $item['course_id'] ) );
			$link           = '<a href="' . $edit_post_link . '">' . $title . '</a>';
		}
