...
class LP_Cert_AJAX {
	...
	public function lp_cert_add_to_cart_woo() {
		$result = array(
			'code'    => 0,
			'message' => __( 'error', 'ecademy-toolkit' ),
		);
		...
		if ( $cart_item_key ) {
			...
			$result['button_view_cart'] = '<a class="btn-lp-cert-view-cart" target="_blank" href="' . wc_get_cart_url() . '"><button class="lp-button">' . __( 'View cart certificate', 'ecademy-toolkit' ) . '</button></a>';
			...
			if ( isset( $wc_notices['error'] ) && ! empty( $wc_notices['error'] ) ) {
				...
				$result['message'] = esc_html__( 'You cannot add this certificate to your cart. Maybe certificate exists', 'ecademy-toolkit' );
			}
		}
