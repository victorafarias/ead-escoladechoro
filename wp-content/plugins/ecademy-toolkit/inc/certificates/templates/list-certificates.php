...
if ( isset( $certificates ) ) {
	?>
	<h3 class="profile-heading"><?php esc_html_e( 'Certificates', 'ecademy-toolkit' ); ?></h3>
	<ul class="learn-press-courses profile-certificates">
		...
		foreach ( $certificates as $course_id => $data ) {
			...
			<?php } elseif ( ! $can_get_cert['flag'] && $can_get_cert['reason'] == 'not_buy' ) { ?>
				<li class="course">
					<p><?php echo sprintf( __( 'In order to get the certificate of the %s course, please pay first!', 'ecademy-toolkit' ), '<a href="' . $course->get_permalink() . '">' . $course->get_title() . '</a>' ); ?></p>
...
<?php } else {
	learn_press_display_message( __( 'No certificates!', 'ecademy-toolkit' ) );
} ?>
