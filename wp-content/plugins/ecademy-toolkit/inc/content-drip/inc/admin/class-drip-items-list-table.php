		...
		public function get_columns() {
			...
			<div class="quick-settings">
				...
				<div class="quick-settings-form">
					<p>
						<label><?php _e( 'Start', 'ecademy-toolkit' ); ?></label>
						<input type="number" name="start" value="1" min="0" max="100" step="0.5">
					</p>
					<p>
						<label><?php _e( 'Step', 'ecademy-toolkit' ); ?></label>
						<input type="number" name="step" value="1" min="0" max="100" step="0.5">
					</p>
					<p>
						<label><?php _e( 'Type', 'ecademy-toolkit' ); ?></label>
						<select name="type">
					...
					<p>
						<button class="button button-primary apply-quick-settings"
								type="button"><?php _e( 'Apply', 'ecademy-toolkit' ); ?></button>
						<button class="button close-quick-settings"
								type="button"><?php _e( 'Close', 'ecademy-toolkit' ); ?></button>
					</p>
				</div>
			</div>
			<?php
			...
			$columns = array(
				...
				'title'        => __( 'Item name', 'ecademy-toolkit' ),
				'type'         => __( 'Type', 'ecademy-toolkit' ),
				'prerequisite' => __( 'Prerequisite', 'ecademy-toolkit' ),
				'delay'        => __( 'Delay access', 'ecademy-toolkit' ) . $html
			);
...
// 			$this->items           = wp_cache_get( 'course-' . $this->course->get_id(), 'lp-course-items' );
		...
		protected function extra_tablenav( $which ) { ?>
			<button class="button button-primary"><?php _e( 'Update', 'ecademy-toolkit' ); ?></button>
			<button class="button" type="button"
					id="learn-press-reset-drip-items"><?php _e( 'Reset', 'ecademy-toolkit' ); ?></button>
			<?php
		}
		...
		protected function handle_row_actions( $post, $column_name, $primary ) {
			...
			if ( $can_edit_post && 'trash' != $post->post_status ) {
				$actions['edit'] = sprintf(
					...
					__( 'Edit', 'ecademy-toolkit' )
				);
				$actions['view'] = sprintf(
					'<a href="%s">%s</a>',
					$this->course->get_item_link( $post->ID ),
					__( 'View', 'ecademy-toolkit' )
				);
			}
...
//			RWMB_Datetime_Field::admin_enqueue_scripts();
			...
			switch ( $column_name ) {
				...
				case 'delay':
					...
					$delay_types = apply_filters( 'lp_drip_delay_types', array(
						'immediately' => __( 'Immediately', 'ecademy-toolkit' ),
						'interval'    => __( 'After...', 'ecademy-toolkit' ),
						'specific'    => __( 'Specific date', 'ecademy-toolkit' )
					) );
		...
		protected function get_item_type( $item ) {
			...
			return __( 'Unknown Type', 'ecademy-toolkit' );
		}
