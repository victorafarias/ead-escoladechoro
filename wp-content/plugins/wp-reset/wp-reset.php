...
class WP_Reset
{
    ...
    public $log = false;
    private $wp_config_constants = false;
    ...
    function admin_actions()
    {
        ...
        if (isset($_GET['wpr_wp_debug'])) {
            if ($_GET['wpr_wp_debug'] == 'true') {
                $this->wpconfig_read_constants();
                $this->wpconfig_toggle_debug(true);
            } else {
                $this->wpconfig_toggle_debug(false);
            }
        }
 ...
 */
    ...
    function ajax_run_tool()
    {
        ...
        } elseif ($tool == 'activate_dropin') {
            $res = $wp_reset_tools->do_activate_dropin($extra_data);
            if (is_wp_error($res)) {
                wp_send_json_error($res->get_error_message());
            } else {
                wp_send_json_success();
            }
        } elseif ($tool == 'deactivate_dropin') {
            $res = $wp_reset_tools->do_deactivate_dropin($extra_data);
            if (is_wp_error($res)) {
                wp_send_json_error($res->get_error_message());
            } else {
                wp_send_json_success();
            }
        } elseif ($tool == 'delete_dropin') {
            $res = $wp_reset_tools->do_delete_dropin($extra_data);
            if (is_wp_error($res)) {
                wp_send_json_error($res->get_error_message());
            } else {
                wp_send_json_success();
            }
        } elseif ($tool == 'delete_mu_plugin') {
            $res = $wp_reset_tools->do_delete_mu_plugin($extra_data);
            if (is_wp_error($res)) {
                wp_send_json_error($res->get_error_message());
            } else {
                wp_send_json_success();
    ...
    function do_reinstall($params = array())
    {
        ...
        $params = shortcode_atts(array('reactivate_webhooks' => false, 'reactivate_theme' => false, 'reactivate_wpreset' => false, 'reactivate_plugins' => false, 'mainwp_reset' => false, 'user_id' => false, 'user_name' => false), (array) $params);
        ...
        if (!empty($params['mainwp_reset'])) {
            $mainwp_child_actions_data_checked = get_option('mainwp_child_actions_data_checked');
            $mainwp_child_actions_saved_data = get_option('mainwp_child_actions_saved_data');
            $mainwp_child_actions_saved_number_of_days = get_option('mainwp_child_actions_saved_number_of_days');
            $mainwp_child_activated_once = get_option('mainwp_child_activated_once');
            $mainwp_child_auth = get_option('mainwp_child_auth');
            $mainwp_child_auto_purge_cache = get_option('mainwp_child_auto_purge_cache');
            $mainwp_child_branding_settings = get_option('mainwp_child_branding_settings');
            $mainwp_child_clone_from_server_last_folder = get_option('mainwp_child_clone_from_server_last_folder');
            $mainwp_child_clone_sites = get_option('mainwp_child_clone_sites');
            $mainwp_child_cloud_flair_enabled = get_option('mainwp_child_cloud_flair_enabled');
            $mainwp_child_connected_admin = get_option('mainwp_child_connected_admin');
            $mainwp_child_nonce = get_option('mainwp_child_nonce');
            $mainwp_child_openssl_sign_algo = get_option('mainwp_child_openssl_sign_algo');
            $mainwp_child_pingnonce = get_option('mainwp_child_pingnonce');
            $mainwp_child_plugin_version = get_option('mainwp_child_plugin_version');
            $mainwp_child_pubkey = get_option('mainwp_child_pubkey');
            $mainwp_child_server = get_option('mainwp_child_server');
            $mainwp_child_siteid = get_option('mainwp_child_siteid');
            $mainwp_child_update_version = get_option('mainwp_child_update_version');
            $mainwp_premium_updates = get_option('mainwp_premium_updates');

            $current_user = get_userdata($params['user_id']);
        }
        ...
        if (!empty($params['mainwp_reset'])) {
            activate_plugin('mainwp-child/mainwp-child.php');

            update_option('mainwp_child_actions_data_checked', $mainwp_child_actions_data_checked);
            update_option('mainwp_child_actions_saved_data', $mainwp_child_actions_saved_data);
            update_option('mainwp_child_actions_saved_number_of_days', $mainwp_child_actions_saved_number_of_days);
            update_option('mainwp_child_activated_once', $mainwp_child_activated_once);
            update_option('mainwp_child_auth', $mainwp_child_auth);
            update_option('mainwp_child_auto_purge_cache', $mainwp_child_auto_purge_cache);
            update_option('mainwp_child_branding_settings', $mainwp_child_branding_settings);
            update_option('mainwp_child_clone_from_server_last_folder', $mainwp_child_clone_from_server_last_folder);
            update_option('mainwp_child_clone_sites', $mainwp_child_clone_sites);
            update_option('mainwp_child_cloud_flair_enabled', $mainwp_child_cloud_flair_enabled);
            update_option('mainwp_child_connected_admin', $params['user_name']);
            update_option('mainwp_child_nonce', $mainwp_child_nonce);
            update_option('mainwp_child_openssl_sign_algo', $mainwp_child_openssl_sign_algo);
            update_option('mainwp_child_pingnonce', $mainwp_child_pingnonce);
            update_option('mainwp_child_plugin_version', $mainwp_child_plugin_version);
            update_option('mainwp_child_pubkey', $mainwp_child_pubkey);
            update_option('mainwp_child_server', $mainwp_child_server);
            update_option('mainwp_child_siteid', $mainwp_child_siteid);
            update_option('mainwp_child_update_version', $mainwp_child_update_version);
            update_option('mainwp_premium_updates', $mainwp_premium_updates);
        }
    ...
    function plugin_page()
    {
        ...
        if ($wp_reset_licensing->is_active()) {
            ...
            echo '<li><a href="#tab-debug">' . __('Debug', 'wp-reset') . '</a></li>';
    ...
    /**
     * Determine the wp-config.php file location
     *
     * @return string wp-config.php absolute path
     */
    private function wpconfig_file()
    {
        if (file_exists(ABSPATH . 'wp-config.php')) {
            ...
            return new WP_Error(1, 'Can\'t find wp-config.php.');
        }

        if (!is_writable($file)) {
            return new WP_Error(1, 'wp-config.php file (' . $file . ') is not writeable');
        }
    ...
    /**
     * Toggle WP_DEBUG and WP_DEBUG_LOG ON/OFF by setting value to true or false
     * If WP_DEBUG is already enabled and WP_DEBUG_LOG is set to a custom path WP Reset will read it from there, 
     * otherwise it will set WP_DEBUG_LOG to true and use the default wp-content/debug.log path
     *
     * @return null
     */
    private function wpconfig_toggle_debug($wpdebug = false)
    {
        if (!current_user_can('administrator')) {
            return new WP_Error(1, 'Can only call as admin');
        }
        
        $wpconfig_file_path = $this->wpconfig_file();

        if (is_wp_error($wpconfig_file_path)) {
            return $wpconfig_file_path;
        }

        $wp_config_src = file_get_contents($wpconfig_file_path);

        $wpconfig_read_constants = $this->wpconfig_read_constants();

        if (is_wp_error($wpconfig_read_constants)) {
            return $wpconfig_read_constants;
        }

        // Enable WP_DEBUG
        if (isset($this->wp_config_constants['WP_DEBUG'])) {
            $wp_config_src = preg_replace(
                sprintf('/(?<=^|;|<\?php\s|<\?\s)(\s*?)%s/m', preg_quote(trim($this->wp_config_constants['WP_DEBUG']['src']), '/')),
                '$1' . 'define(\'WP_DEBUG\', ' . ($wpdebug ? 'true' : 'false') . ' );',
                $wp_config_src
            );
        } else {
            //add
            ...
            if ('EOF' === $anchor) {
                $wp_config_src = $wp_config_src . PHP_EOL . 'define(\'WP_DEBUG\', ' . ($wpdebug ? 'true' : 'false') . ' );';
            } else {
                if (false === strpos($wp_config_src, $anchor)) {
                    return new WP_Error(1, 'Updating wp-config.php file failed. No changes have been made.');
                }

                $new_src  = 'define(\'WP_DEBUG\', ' . ($wpdebug ? 'true' : 'false') . ' );' . PHP_EOL . $anchor;
                $wp_config_src = str_replace($anchor, $new_src, $wp_config_src);
            }
        }

        // Enable WP_DEBUG_LOG
        if (isset($this->wp_config_constants['WP_DEBUG_LOG'])) {
            $wp_config_src = preg_replace(
                sprintf('/(?<=^|;|<\?php\s|<\?\s)(\s*?)%s/m', preg_quote(trim($this->wp_config_constants['WP_DEBUG_LOG']['src']), '/')),
                '$1' . 'define(\'WP_DEBUG_LOG\', ' . ($wpdebug ? 'true' : 'false') . ' );',
                $wp_config_src
            );
        } else {
            //add
            ...
            if ('EOF' === $anchor) {
                $wp_config_src = $wp_config_src . PHP_EOL . 'define(\'WP_DEBUG_LOG\', ' . ($wpdebug ? 'true' : 'false') . ' );';
            } else {
                if (false === strpos($wp_config_src, $anchor)) {
                    return new WP_Error(1, 'Updating wp-config.php file failed. No changes have been made.');
                }

                $new_src  = 'define(\'WP_DEBUG_LOG\', ' . ($wpdebug ? 'true' : 'false') . ' );' . PHP_EOL . $anchor;
                $wp_config_src = str_replace($anchor, $new_src, $wp_config_src);
            }
        }

        $result = file_put_contents($wpconfig_file_path, $wp_config_src, LOCK_EX);

        if (false === $result) {
            return new WP_Error(1, 'Writing wp-config.php file failed. No changes have been made.');
        }
    ...
    /**
     * Read constants from wp-config.php
     *
     * @return array of constants as keys and values 
     */
    public function wpconfig_read_constants($force = false)
    {
        if (!current_user_can('administrator')) {
            return new WP_Error(1, 'Can only call as admin');
        }
        
        if ($this->wp_config_constants !== false && !$force) {
            return $this->wp_config_constants;
        }
        ...
        $src = file_get_contents($wpconfig_file_path);
        ...
        if (!empty($constants[0]) && !empty($constants[1]) && !empty($constants[2]) && !empty($constants[3]) && !empty($constants[4]) && !empty($constants[5])) {
            foreach ($constants[2] as $index => $name) {
                $this->wp_config_constants[$name] = array(
                    'src'   => $constants[0][$index],
                    'val' => $constants[4][$index]
                );
            }
        }

        if (empty($this->wp_config_constants)) {
            return new WP_Error(1, 'wp-config.php file could not be read correctly');
        }
    ...
    function file_tail($filepath, $lines = 500) {
        $f = fopen($filepath, "r");
        ...
        while ($position > 0 && $lines_found <= $lines) {
            $position = max($position - $buffer, 0);
            fseek($f, $position);
            $chunk = fread($f, $buffer) . $chunk;
    ...
    /**
     * Echoes content for WordPress log
     *
     * @return null
     */
    private function print_wordpress_debug_log()
    {
        $log_file = defined('WP_DEBUG_LOG') && !is_bool(WP_DEBUG_LOG) ? WP_DEBUG_LOG : WP_CONTENT_DIR . '/debug.log';
        
        if (file_exists($log_file)) {
            $log_lines = $this->file_tail($log_file, 500);
            ...
            if(count($log_lines) == 0){
                echo 'Debug log is empty. Refresh the page to reload it.';
            }
            ...
            foreach ($log_lines as $line) {
                ...
                if (count($matches) >= 4) {
                    $date = $matches[1];
                    $level = $matches[2];
                    $message = htmlspecialchars($matches[3]); // Escape HTML characters for security
                    $class = isset($log_levels[$level]) ? $log_levels[$level] : 'other';

                    // Output log row with date, log level, and message
                    echo '<div class="wpr-log-row">';
                    echo '<div class="wpr-log-date">' . $date . '<br /><div class="wpr-log-' . $class . '">' . $level . '</div></div>';
                    echo '<div class="wpr-log-message">' . $message . '</div>';
                    echo '</div>';
                }
            }
        } else {
    ...
    private function tab_debug()
    {
        ...
        $options = $this->get_options();
        $plugin_name = $this->get_rebranding('name');
        $this->wpconfig_read_constants(true);
        ...
        echo '<div class="card">';
        echo '<h4><span class="card-name">WordPress Debug Log</span><div class="card-header-right"></div></h4>';
        echo '<div class="card-body">';
        echo '<p>The WP_DEBUG feature in WordPress allows developers to debug their code by displaying error messages, notices, and warnings directly on the website. However, accessing these debug messages typically requires direct access to the server\'s error logs. This feature aims to simplify the debugging process by providing users with a convenient way to enable WP_DEBUG and view debug log contents directly from the WordPress admin panel.</p>';

        if (is_wp_error($this->wp_config_constants)) {
            echo 'There has been an error reading your wp-config.php file: ' . $this->wp_config_constants->get_error_message();
        } else {

            if ($this->wp_config_constants['WP_DEBUG']['val'] !== 'true') {
                echo '<a href="' . admin_url('tools.php?page=wp-reset&wpr_wp_debug=true') . '" data-confirm-title="Are you sure you want to enable WordPress Debug mode?" data-text-confirm="Debug mode will enable the WordPress debug by setting WP_DEBUG to true and you will be able to view the debug.log in the WP Reset Debug tab." data-btn-confirm="Enable WordPress debug mode" class="enable-debug-mode button button-delete">Enable WordPress Debug</a>';
            } else {
                echo '<p>WordPress Debug is enabled. Press the <strong>Disable WordPress Debug</strong> button below to disable it. If you need to disable it manually, you can follow these instructions:</p>';
                echo '<p>
                    <ol>
                    <li>Access the WordPress installation directory on the server.</li>
                    <li>Locate the wp-config.php file, which contains essential configuration settings for the WordPress site.</li>
                    <li>Open the wp-config.php file using a text editor or file manager.</li>
                    <li>Search for the line that defines the WP_DEBUG constant. It typically looks like this: define("WP_DEBUG", true);</li>
                    <li>To disable WP_DEBUG, simply change the value from true to false, like so: define("WP_DEBUG", false);</li>
                    <li>Save the changes to the wp-config.php file and close the editor.</li>
                    </ol>';
                echo '</p>';
                echo '<a href="' . admin_url('tools.php?page=wp-reset&wpr_wp_debug=false') . '" class="button button-delete disable-debug-mode">Disable WordPress Debug</a>';

                echo '<div class="wpr-log-wrapper">';
                echo $this->print_wordpress_debug_log();
                echo '</div>';
            }
        }
        ...
        echo '<div class="card">';
        echo '<h4><span class="card-name">' . $plugin_name . ' Debug</span><div class="card-header-right"></div></h4>';
        echo '<div class="card-body">';
        echo '<p class="red">Only use the ' . $plugin_name . ' Debug Mode if you know what you are doing or have been instructed to do so by support. While debug mode is enabled, any PHP notices and errors will be printed in AJAX responses (if WordPress Debug Mode is enabled, then they will only appear in the debug log), so you should not leave the Debug mode enabled during normal use.</p>';
    ...
    private function tab_tools()
    {
        ...
        $tools = array(
            ...
            'tool-manage-mu-plugins' => 'Manage MU Plugins',
            'tool-manage-dropins' => 'Manage Drop-ins',
        ...
        if (is_multisite()) {
            ...
            if (!empty($mu_plugins)) {
                ...
                foreach ($mu_plugins as $mu_file => $mu_plugin) {
                    echo '<li>';
                    echo $mu_plugin['Name'] . ' ';
                    echo ' <a data-confirm-title="Are you sure you want to delete the ' . $mu_plugin['Name'] . ' must-use plugin?" data-btn-confirm="Delete must-use plugin" data-text-wait="Deleting must-use plugin. Please wait." data-text-confirm="Must-use plugin will be deleted. There is NO UNDO. ' . $plugin_name . ' does not make any file backups." data-text-done="Must-use plugin has been deleted. Click OK to reload the page." data-mu-file="' . $mu_file . '" class="button-delete delete-mu-plugin" href="#">Delete</a>';
                    echo '</li>';
                } // foreach
                echo '</ul>';

                echo '<p class="mb0"><a data-confirm-title="Are you sure you want to delete all must-use plugins?" data-btn-confirm="Delete must-use plugins" data-text-wait="Deleting Must-Use plugins. Please wait." data-text-confirm="All must-use plugins will be deleted. There is NO UNDO. ' . $plugin_name . ' does not make any file backups." data-text-done="%n must-use plugins have been deleted." data-text-done-singular="One must-use plugin has been deleted." class="button button-delete" href="#" id="delete-mu-plugins">Delete all must-use plugins</a></p>';
            } else {
                echo '<p>No must-use plugins found.</p>';
            }
        }
        ...
        echo '<div class="card">';
        echo $this->get_card_header(__('Manage Drop-ins', 'wp-reset'), 'tool-manage-dropins', array('collapse_button' => true, 'iot_button' => true, 'documentation_link' => true));
        echo '<div class="card-body">';
        echo '<p>Drop-ins are pieces of code found in <code>/wp-content/</code> that replace default, built-in WordPress functionality. Most often used are <code>db.php</code> and <code>advanced-cache.php</code> that implement custom DB and cache functionality. They can\'t be deactivated via the <a href="' . admin_url('plugins.php?plugin_status=dropins') . '" target="_blank">plugins interface</a> but if any are present they are listed in the "Drop-in" tab.<br>Disabled drop-ins are renamed to: <code>_disabled_wpr_original_filename.php</code></p>';
        ...
        if (is_multisite()) {
            ...
            foreach ($dropins as $dropin_file => $dropin) {
                if (($active_dropin = file_exists(trailingslashit(WP_CONTENT_DIR) . $dropin_file)) ||
                    file_exists(trailingslashit(WP_CONTENT_DIR) . '_disabled_wpr_' . $dropin_file)
                ) {
                    $has_dropins = true;

                    if ($active_dropin) {
                        $plugin_details = get_plugin_data(trailingslashit(WP_CONTENT_DIR) . $dropin_file);
                    } else {
                        $plugin_details = get_plugin_data(trailingslashit(WP_CONTENT_DIR) . '_disabled_wpr_' . $dropin_file);
                    }
                    echo '<li>';
                    if (!empty($plugin_details['Name'])) {
                        echo $plugin_details['Name'] . ' <code>' . $dropin_file . '</code>';
                    } else {
                        echo $dropin[0] . ' <code>' . $dropin_file . '</code>';
                    }
                    if ($active_dropin) {
                        echo ' <a data-confirm-title="Are you sure you want to deactivate the ' . $dropin_file . ' drop-in?" data-btn-confirm="Deactivate drop-in" data-text-wait="Deactivating drop-in. Please wait." data-text-confirm="If the site becomes unresponsive, and you can\'t access WP admin use the Emergency Recovery Script to regain access." data-text-done="Drop-in has been deactivated. Click OK to reload the page." data-dropin-file="' . $dropin_file . '" class="deactivate-dropin" href="#">Deactivate</a>';
                    } else {
                        echo ' <a data-confirm-title="Are you sure you want to activate the ' . $dropin_file . ' drop-in?" data-btn-confirm="Activate drop-in" data-text-wait="Activating drop-in. Please wait." data-text-confirm="If the site becomes unresponsive, and you can\'t access WP admin use the Emergency Recovery Script to regain access." data-text-done="Drop-in has been activated. Click OK to reload the page." data-dropin-file="_disabled_wpr_' . $dropin_file . '" class="activate-dropin" href="#">Activate</a>';
                    }
                    echo ' <a data-confirm-title="Are you sure you want to delete the ' . $dropin_file . ' drop-in?" data-btn-confirm="Delete drop-in" data-text-wait="Deleting drop-in. Please wait." data-text-confirm="Drop-in will be deleted. There is NO UNDO. ' . $plugin_name . ' does not make any file backups." data-text-done="Drop-in has been deleted. Click OK to reload the page." data-dropin-file="' . (!$active_dropin ? '_disabled_wpr_' : '') . $dropin_file . '" class="button-delete delete-dropin" href="#">Delete</a>';
                    echo '</li>';
                }
            } // foreach dropins
            echo '</ul>';
            ...
            if ($has_dropins) {
                echo '<p class="mb0"><a data-confirm-title="Are you sure you want to delete all drop-ins?" data-btn-confirm="Delete drop-ins" data-text-wait="Deleting drop-ins. Please wait." data-text-confirm="All drop-ins will be deleted. There is NO UNDO. ' . $plugin_name . ' does not make any file backups." data-text-done="%n drop-ins have been deleted." data-text-done-singular="One drop-in has been deleted." class="button button-delete" href="#" id="delete-dropins">Delete all drop-ins</a></p>';
            } else {
    ...
    private function tab_support()
    {
        ...
        if (WP_Reset_Utility::whitelabel_filter()) {
            ...
            if ($this->get_rebranding() === false) {
                ...
                echo '<div class="card">';
                echo $this->get_card_header('MainWP WP Reset Extension', 'documentation-support', array());
                echo '<div class="card-body">';
                echo '<p>You can download the WP Reset PRO MainWP Extension, install and activate it on your MainWP Dashboard, then in your MainWP Dashboard you can access it under Extensions -> WP Reset PRO.</p>';
                echo '<p>Child websites that have WP Reset PRO version 6.14 or newer installed will show WP Reset information and allow you to reset them remotely.</p>';
                echo '<p class="red">Please note that the extension is in beta, so use it with caution.<p>';
                echo '<p><a href="https://dashboard.wpreset.com/wp-content/uploads/2024/03/wp-reset-mainwp-extension.zip">Download the WP Reset PRO MainWP extension.</a></p>';
                echo '</div>';
                echo '</div>';
    ...
    function get_all_snapshots()
    {
        ...
        $cloud_snapshots = $wp_reset_cloud->get_cloud_snapshots();
        if (is_array($cloud_snapshots) && is_array($snapshots)) {
    ...
    function mainwp_child_extra_execution($info, $post_data)
    {
        global $wp_reset_licensing;
        $license = $this->get_license();
        $snapshots = $this->get_all_snapshots();
        ...
        if ($wp_reset_licensing->is_active()) {
            $license_key = (empty($license['license_key']) ? '12345678-12345678-12345678-12345678' : substr(esc_attr($license['license_key']), 0, 8) . '-************************');
        } else {
            $license_key = 'inactive';
        }
        ...
        foreach ($admins as $admin) {
            $admin_array[$admin->ID] = $admin->user_login;
        }
            ...
            switch ($post_data['action']) {
                case 'wp_reset_info':
                    $tests = array(
                        ...
                        'admins' => $admin_array,
                        'theme' => $theme_name,
                        'post' => $post_data['extra_param'],
                        'snapshots' => array(
                            'total' => WP_Reset_Utility::format_size($space_usage_total),
                            'user' => WP_Reset_Utility::format_size($space_usage_user),
                            'auto' => WP_Reset_Utility::format_size($space_usage_auto)
                        )
                    );
                ...
                case 'wp_reset_reset':
                    ...
                    if ($post_data['reactivate_theme'] == 'true') {
                        $extra_data['reactivate_theme'] = 'true';
                    }

                    if ($post_data['reactivate_wpreset'] == 'true') {
                        $extra_data['reactivate_wpreset'] = 'true';
                    }

                    if ($post_data['reactivate_plugins'] == 'true') {
                        $extra_data['reactivate_plugins'] = 'true';
                    }
