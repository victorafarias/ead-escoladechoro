...
define( 'USER_PROFILE_PICTURE_DIR', plugin_dir_path( __FILE__ ) );
define( 'USER_PROFILE_PICTURE_URL', plugins_url( '/', __FILE__ ) );
define( 'USER_PROFILE_PICTURE_SLUG', plugin_basename( __FILE__ ) );
define( 'USER_PROFILE_PICTURE_FILE', __FILE__ );
...
class User_Profile_Picture {
    ...
    public function __construct() {
        ...
        if ( 'on' === $options['load_gutenberg'] ) {
            ...
            new User_Profile_Picture_Gutenberg();
        }
    ...
    public function add_block_category( $categories, $post ) {
        return array_merge(
            ...
            array(
                array(
                    'slug'  => 'mpp',
                    'title' => __( 'User Profile Picture', 'profile-builder' ),
                    'icon'  => 'groups',
                ),
            )
        );
    ...
    public function register_settings_submenu_page() {
        add_submenu_page( 'profile-builder', __( 'User Profile Picture', 'profile-builder' ), __( 'User Profile Picture', 'profile-builder' ), apply_filters( 'wppb_user_profile_picture_page_capability', 'manage_options' ), 'profile-user-profile-picture', array( $this, 'admin_page' ) );
    }
    public function register_settings_menu_page() {
        if ( defined( 'USER_PROFILE_PICTURE_ENHANCED' ) ) {
            $hook = add_menu_page(
                __( 'User Profile Picture', 'profile-builder' ),
                __( 'User Profile Picture', 'profile-builder' ),
                'manage_options',
                'mpp',
                array( $this, 'upp_admin_page' ),
                'dashicons-groups',
                100
            );
        } else {
...
//    }
    ...
    /**
     * Old admin page redirect for User Profile Picture
     *
     * @since 2.3.0
     */
    public function upp_admin_page() {
//        $redirect = self_admin_url('admin.php?page=profile-user-profile-picture');
//        wp_safe_redirect($redirect);
    ...
    function admin_page_redirect() {
        global $pagenow;
        if ( $pagenow == 'options-general.php' && isset( $_GET['page'] ) && $_GET['page'] == 'mpp' ) {
            wp_redirect( self_admin_url('admin.php?page=profile-user-profile-picture') );
            exit;
        }
    }
    ...
    public function admin_page() {
        if ( isset( $_POST['submit'] ) && isset( $_POST['options'] ) ) {
            ...
            printf( '<div class="updated"><p><strong>%s</strong></p></div>', esc_html__( 'Your options have been saved.', 'profile-builder' ) );
        }
    ...
    public function ajax_add_thumbnail() {
        ...
        if ( has_post_thumbnail( $post_id ) ) {
            ...
            $post_thumbnail = sprintf( '<img src="%s" width="150" height="150" title="%s" />', esc_url( $thumb_src[0] ), esc_attr__( 'Upload or Change Profile Picture', 'profile-builder' ) );
            $crop_html      = $this->get_post_thumbnail_editor_link( $post_id );
            $thumb_html     = sprintf( '<a href="#" class="mpp_add_media">%s%s</a>', $post_thumbnail, sprintf( '<div id="metronet-click-edit">%s</div>', esc_html__( 'Click to Edit', 'profile-builder' ) ) );
            $thumb_html    .= sprintf( '<a id="metronet-remove" class="dashicons dashicons-trash" href="#" title="%s">%s</a>', esc_attr__( 'Remove profile image', 'profile-builder' ), esc_html__( 'Remove profile image', 'profile-builder' ) );
            wp_send_json(
                array(
    ...
    public function ajax_get_thumbnail() {
        ...
        if ( has_post_thumbnail( $post_id ) ) {
            $thumb_src      = wp_get_attachment_image_src( get_post_thumbnail_id( $post_id ), 'thumbnail', false, '' );
            $post_thumbnail = sprintf( '<img style="display:block" src="%s" width="150" height="150" title="%s" />', esc_url( $thumb_src[0] ), esc_attr__( 'Upload or Change Profile Picture', 'profile-builder' ) );
            $crop_html      = $this->get_post_thumbnail_editor_link( $post_id );
            $thumb_html     = sprintf( '<a href="#" class="mpp_add_media">%s%s</a>', $post_thumbnail, sprintf( '<div id="metronet-click-edit">%s</div>', esc_html__( 'Click to Edit', 'profile-builder' ) ) );
            $thumb_html    .= sprintf( '<a id="metronet-remove" class="dashicons dashicons-trash" href="#" title="%s">%s</a>', esc_attr__( 'Remove profile image', 'profile-builder' ), esc_html__( 'Remove profile image', 'profile-builder' ) );
            wp_send_json(
    ...
    private function get_post_thumbnail_editor_link( $post_id ) {
        ...
        if ( has_post_thumbnail( $post_id ) && defined( 'PTE_VERSION' ) ) {
            ...
            printf( ' - <a href="%s">%s</a>', esc_url( $pte_url ), esc_html__( 'Crop Thumbnail', 'profile-builder' ) );
        }
        return ob_get_clean();
    }
    ...
    public function init() {
        ...
        add_action( 'admin_menu', array( $this, 'register_settings_submenu_page' ) );
        add_action( 'admin_menu', array( $this, 'admin_page_redirect' ) );
        add_action( 'plugin_action_links_' . USER_PROFILE_PICTURE_SLUG, array( $this, 'plugin_settings_link' ) );
...
//                esc_html__( 'Settings', 'profile-builder' )
//            );
//        } else {
//            $admin_anchor = sprintf(
//                '<a href="%s">%s</a>',
...
//        }
        ...
        $admin_anchor = sprintf(
            '<a href="%s">%s</a>',
            esc_url( admin_url( 'admin.php?page=profile-user-profile-picture' ) ),
            esc_html__( 'Settings', 'profile-builder' )
        );
    ...
    public function insert_upload_form() {
        ...
        ?>
        <tr valign="top">
            <th scope="row"><?php esc_html_e( 'Profile Image', 'profile-builder' ); ?></th>
            <td id="mpp">
                ...
                <div id="metronet-profile-image">
                    ...
                    if ( has_post_thumbnail( $post_id ) ) {
                        ...
                        echo sprintf( '<div id="metronet-click-edit">%s</div>', esc_html__( 'Click to Edit', 'profile-builder' ) );
                        echo '</a>';
                    } else {
                        echo '<a style="display:block" href="#" class="mpp_add_media default-image">';
                        $post_thumbnail = sprintf( '<img style="display:block" src="%s" width="150" height="150" title="%s" />', self::get_plugin_url( 'img/mystery.png' ), esc_attr__( 'Upload or Change Profile Picture', 'profile-builder' ) );
                        echo wp_kses_post( $post_thumbnail );
                        echo sprintf( '<div id="metronet-click-edit">%s</div>', esc_html__( 'Click to Edit', 'profile-builder' ) );
                        echo '</a>';
                    }
                    ...
                    ?>
                    <a id="metronet-remove" class="<?php echo implode( ' ', $remove_classes ); // phpcs:ignore ?>" href="#" title="<?php esc_attr_e( 'Remove profile image', 'profile-builder' ); ?>"><?php esc_html_e( 'Remove profile image', 'profile-builder' ); ?></a>
                ...
                <div id="metronet-override-avatar">
                    ...
                    <?php
                        ...
                        ?>
                        <br /><input type="checkbox" name="metronet-user-avatar" id="metronet-user-avatar" value="on" <?php echo $checked; // phpcs:ignore ?> /> <label for="metronet-user-avatar"><?php esc_html_e( 'Override Avatar?', 'profile-builder' ); ?></label>
                    <?php endif; ?>
    ...
    public function print_media_scripts() {
        ...
        wp_enqueue_script( 'mt-pp', self::get_plugin_url( '/js/mpp.js' ), $script_deps, USER_PROFILE_PICTURE_VERSION, true );
        wp_localize_script(
            'mt-pp',
            'metronet_profile_image',
            array(
                'set_profile_text'    => __( 'Set Profile Image', 'profile-builder' ),
                'remove_profile_text' => __( 'Remove Profile Image', 'profile-builder' ),
                'crop'                => __( 'Crop Thumbnail', 'profile-builder' ),
    ...
    public function rest_api_get_posts_for_user( $request ) {
        ...
        $user    = get_user_by( 'id', $user_id );
        if ( ! $user ) {
            return new WP_Error( 'mpp_no_user', __( 'User not found.', 'profile-builder' ), array( 'status' => 404 ) );
        }
    ...
    public function rest_api_get_profile_for_user( $object, $field_name, $request ) {
        ...
        $post_thumbnail_id = get_post_thumbnail_id( $profile_post_id );
        if ( ! $post_thumbnail_id ) {
            return new WP_Error( 'mpp_no_profile_picture', __( 'Profile picture not found.', 'profile-builder' ), array( 'status' => 404 ) );
        }
...
global $mt_pp;
if ( class_exists( 'User_Profile_Picture' ) ) {
    if ( get_bloginfo( 'version' ) >= '3.5' ) {

        $plugin_names = array(
            'metronet-profile-picture/metronet-profile-picture.php',
            'user-profile-picture-dev/metronet-profile-picture.php',
            'user-profile-picture/metronet-profile-picture.php'
        );
        ...
        foreach ( $plugin_names as $plugin_name ) {
            ...
            $network_deactivated_plugin = false;
            ...
            else if ( ( is_plugin_active_for_network( $plugin_name ) ) ) {
                deactivate_plugins( $plugin_name, false, true );

                $network_deactivated_plugin = true;
            }

            if ( $deactivated_plugin || $network_deactivated_plugin ) {
                $wppb_free_add_ons_settings['user-profile-picture'] = true;


                $wppb_deactivated_upp = get_option( 'wppb_deactivated_upp', array() );
                $wppb_deactivated_upp [time()] = array(
                    'upp_type'    => $plugin_name,
                    'pb_version'  => PROFILE_BUILDER_VERSION,
                    'upp_version' => METRONET_PROFILE_PICTURE_VERSION,
                );

                update_option( 'wppb_deactivated_upp', $wppb_deactivated_upp );
                return;
            }

            add_action( 'activate_' . $plugin_name, 'wppb_user_profile_picture_prevent_activation' );

        }

        upp_instantiate();
//        add_action( 'plugins_loaded', 'upp_instantiate', 10 );
    }
}

function wppb_user_profile_picture_prevent_activation()
{
    ...
    if ( array_key_exists( 'user-profile-picture', $wppb_free_add_ons_settings ) ) {
        if ( $wppb_free_add_ons_settings['user-profile-picture'] == true ) {
            $redirect = self_admin_url('plugins.php?user-profile-picture-error=1');
        } else {
            $redirect = self_admin_url('plugins.php?user-profile-picture-error=2');
        }
 ...
 */
function upp_instantiate() {
    global $mt_pp;
    if ( !$mt_pp ) {
        $mt_pp = new User_Profile_Picture();
        do_action( 'user_profile_picture_loaded' );
    }
}

add_action( 'admin_notices', 'wppb_user_profile_picture_prevent_activation_admin_notice' );

function wppb_user_profile_picture_prevent_activation_admin_notice() {
    if ( isset( $_GET['user-profile-picture-error'] ) ) {
        switch ( $_GET['user-profile-picture-error'] ) {
            case '1':
                echo '<div class="error"><p>The User Profile Picture plugin cannot be activated while the User Profile Picture add-on for Profile Builder is enabled.</p></div>';
                break;
            case '2':
                echo '<div class="error"><p>The User Profile Picture plugin cannot be activated while Profile Builder is activated. Its functionality has been moved within Profile Builder via the free User Profile Picture add-on which has been enabled.</p></div>';
                break;
            default:
                echo '<div class="error"><p>The User Profile Picture plugin cannot be activated.</p></div>';
        }
    }
}
 ...
 */
function upp_profile_img( $user_id, $args = array() ) {
    $profile_post_id = absint( get_user_option( 'metronet_post_id', $user_id ) );
 ...
 */
function upp_author_box( $user_id = 0, $attributes = array() ) {
    $user = get_user_by( 'id', $user_id );
    if ( false === $user ) {
        return '';
    }
    $defaults = array(
        ...
        'tabbedAuthorProfile'             => __( 'Author', 'profile-builder' ),
        'tabbedAuthorLatestPosts'         => __( 'Latest Posts', 'profile-builder' ),
        'tabbedAuthorProfileHeading'      => __( 'Author Information', 'profile-builder' ),
        ...
        <div class="mpp-enhanced-profile-wrap mpp-block-profile <?php echo 'compact' === $attributes['theme'] ? esc_attr( $attributes['profileCompactAlignment'] ) : ''; ?> <?php echo esc_attr( $attributes['theme'] ); ?> <?php echo esc_attr( $attributes['profileAvatarShape'] ); ?>" style="<?php echo $attributes['padding'] > 0 ? 'padding: ' . esc_attr( $attributes['padding'] ) . 'px;' : ''; ?><?php echo $attributes['border'] > 0 ? 'border:' . esc_attr( $attributes['border'] ) . 'px solid ' . esc_attr( $attributes['borderColor'] ) . ';' : ''; ?><?php echo $attributes['borderRounded'] > 0 ? 'border-radius:' . esc_attr( $attributes['borderRounded'] ) . 'px;' : ''; ?>background-color: <?php echo esc_attr( $attributes['profileBackgroundColor'] ) . ';'; ?> color: <?php echo esc_attr( $attributes['profileTextColor'] ) . ';'; ?>">
            <div class="mpp-profile-gutenberg-wrap mt-font-size-<?php echo esc_attr( $attributes['profileFontSize'] ); ?>">
                            ...
                            <div class="mpp-gutenberg-view-posts">
                                <?php if ( $attributes['showViewPosts'] ) : ?>
                                    <div class="mpp-profile-view-posts" style="background-color: <?php echo esc_attr( $attributes['profileViewPostsBackgroundColor'] ); ?>; color: <?php echo esc_attr( $attributes['profileViewPostsTextColor'] ); ?>; <?php ( '' !== $attributes['website'] && $attributes['showWebsite'] ) ? '' : 'width:' . esc_attr( $attributes['showPostsWidth'] ) . ';'; ?> font-size: <?php echo esc_attr( $attributes['buttonFontSize'] ); ?>px;">
                                        <a href="<?php echo esc_url( $attributes['profileURL'] ); ?>" style="background: <?php echo esc_attr( $attributes['profileViewPostsBackgroundColor'] ); ?>; color: <?php echo esc_attr( $attributes['profileViewPostsTextColor'] ); ?>">
                                            <?php esc_html_e( 'View Posts', 'profile-builder' ); ?></a>
                                ...
                                <?php if ( '' !== $attributes['website'] && $attributes['showWebsite'] ) : ?>
                                    <div class="mpp-profile-view-website" style="background: <?php echo esc_attr( $attributes['profileWebsiteBackgroundColor'] ); ?>;color: <?php echo esc_attr( $attributes['profileWebsiteTextColor'] ); ?>; font-size: <?php echo esc_attr( $attributes['buttonFontSize'] ); ?>px;">
                                        <a href="<?php echo esc_url( $attributes['website'] ); ?>" style="background: <?php echo esc_attr( $attributes['profileWebsiteBackgroundColor'] ); ?>; color: <?php echo esc_attr( $attributes['profileWebsiteTextColor'] ); ?>;"><?php esc_html_e( 'View Website', 'profile-builder' ); ?></a>
                    ...
                    <div class="mpp-profile-meta" style="font-size: <?php echo esc_attr( $attributes['fontSize'] ); ?>px;">
                        <?php if ( $attributes['showViewPosts'] ) : ?>
                            <div class="mpp-profile-link alignleft">
                                <a href="<?php echo esc_url( $attributes['profileURL'] ); ?>" style="color: <?php echo esc_attr( $attributes['profileLinkColor'] ); ?>;"><?php esc_html_e( 'View all posts by', 'profile-builder' ); ?> <?php echo esc_html( $attributes['profileName'] ); ?></a>
                            ...
                            <div class="mpp-profile-link alignright">
                                <a href="<?php echo esc_url( $attributes['website'] ); ?>" style="color: <?php echo esc_attr( $attributes['profileLinkColor'] ); ?>">
                                    <?php esc_html_e( 'Website', 'profile-builder' ); ?>
                                </a>
                            </div><!-- .mpp-profile-link -->
                        <?php endif; ?>
                    ...
                    <div class="mpp-compact-meta">
                        <?php if ( $attributes['showViewPosts'] ) : ?>
                            <div class="mpp-profile-view-posts" style="background: <?php echo esc_attr( $attributes['profileViewPostsBackgroundColor'] ); ?>; color: <?php echo esc_attr( $attributes['profileViewPostsTextColor'] ); ?>; width: 90%; margin: 0 auto 10px auto; font-size: <?php echo esc_attr( $attributes['buttonFontSize'] ); ?>px;">
                                <a href="<?php echo esc_url( $attributes['profileURL'] ); ?>" style="color: <?php echo esc_attr( $attributes['profileViewPostsTextColor'] ); ?>; background: <?php echo esc_attr( $attributes['profileViewPostsBackgroundColor'] ); ?>;"><?php esc_html_e( 'View Posts', 'profile-builder' ); ?></a>
                            </div><!-- .mpp-profile-view-posts -->
                        <?php endif; ?>
                        <?php if ( '' !== $attributes['website'] && $attributes['showWebsite'] ) : ?>
                            <div class="mpp-profile-view-website" style="background: <?php echo esc_attr( $attributes['profileWebsiteBackgroundColor'] ); ?>; color: <?php echo esc_attr( $attributes['profileWebsiteTextColor'] ); ?>; width: 90%; margin: 0 auto 0 auto; font-size: <?php echo esc_attr( $attributes['buttonFontSize'] ); ?>px;">
                                <a href="<?php echo esc_url( $attributes['website'] ); ?>" style="color: <?php echo esc_attr( $attributes['profileWebsiteTextColor'] ); ?>; background: <?php echo esc_attr( $attributes['profileWebsiteBackgroundColor'] ); ?>;"><?php esc_html_e( 'View Website', 'profile-builder' ); ?></a>
                            </div><!-- .mpp-profile-view-posts -->
                        <?php endif; ?>
                ...
                <?php if ( true === $attributes['showSocialMedia'] && ( 'regular' === $attributes['theme'] || 'compact' === $attributes['theme'] || 'profile' === $attributes['theme'] ) ) : ?>
                    <?php echo upp_get_social_icons( $attributes ); // phpcs:ignore ?>
                <?php endif; ?>
    ...
    <?php endif; ?>
    <?php
    wp_enqueue_style( 'mpp_gutenberg', User_Profile_Picture::get_plugin_url( '/css/front-end-gutenberg.css' ), array(), USER_PROFILE_PICTURE_VERSION, 'all' );
    wp_enqueue_script( 'mpp_gutenberg_tabs', User_Profile_Picture::get_plugin_url( 'js/mpp-frontend' . $min_or_not . '.js' ), array( 'jquery' ), USER_PROFILE_PICTURE_VERSION, true );
    add_action( 'wp_footer', 'upp_load_gutenblock_svgs' );
    echo ob_get_clean(); // phpcs:ignore
}
/**
 * Get social icons based on passed attributes
 *
 * @see upp_author_box for attribute valies
 *
 * @since 2.2.0
 *
 * @param array $attributes See defaults in function upp_author_box for all attributes.
 *
 * @return string User social icons
 */
function upp_get_social_icons( $attributes ) {
    ob_start();
    ?>
 ...
 * @since 2.2.0
 */
function upp_load_gutenblock_svgs() {
    /**
