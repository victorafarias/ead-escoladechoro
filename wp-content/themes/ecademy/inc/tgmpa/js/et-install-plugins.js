...
(function ($) {
	$(document).ready(function () {
		...
		var $plugins = $('.ecademy-plugins'),
			$btn = $('.ecademy-admin-btn');

		// install action
		$plugins.on('click', '.ecademy-admin-btn[data-plugin-action="install"]', function (e) {

			e.preventDefault();

			if ($('.ecademy-admin-btn').hasClass('installing')) {
				return;
			}
			...
			$.ajax({
				...
				$btn.css('opacity', '1');
				if ($this.closest('.ecademy-plugin').length) {
					$this.closest('.row-actions').replaceWith('<div class="row-actions visible active"><span class="activate"><a href="#" class="button ecademy-admin-btn">Activated</a></span></div>');
				} else {
					$this.removeClass('installing').attr('data-plugin-action', 'deactivate').attr('href', response.substr(response.lastIndexOf('ecademy') + 6, response.length)).text('Deactivate').closest('.theme').addClass('active');
				}
			}).fail(function () {
		...
		// update action
		$plugins.on('click', '.ecademy-admin-btn[data-plugin-action="update"]', function (e) {
			e.preventDefault();
		...
		// activate action
		$plugins.on('click', '.ecademy-admin-btn[data-plugin-action="activate"]', function (e) {
			...
			var $this = $(this),
				q_href = $this.attr('href').split('&'),
				data = {
					'action': 'ecademy_activate_plugin',
					'plugin': q_href[1].substr(q_href[1].lastIndexOf('=') + 1, q_href[1].length),
		...
		// deactivate action
		$plugins.on('click', '.ecademy-admin-btn[data-plugin-action="deactivate"]', function (e) {
			...
			var $this = $(this),
				q_href = $this.attr('href').split('&'),
				data = {
					'action': 'ecademy_deactivate_plugin',
					'plugin': q_href[1].substr(q_href[1].lastIndexOf('=') + 1, q_href[1].length),
		...
		$('.whi-install-plugins').on('click', function (e) {
			...
			var $allPlugins = $installPluginsBtn.parent().next('.ecademy-plugins');
			var plugins = [];

			$allPlugins.find('.ecademy-plugin:not(:hidden)').each(function (index, element) {
				var $this = $(this);
				var $pluginActionBtn = $this.find('.ecademy-admin-btn');
